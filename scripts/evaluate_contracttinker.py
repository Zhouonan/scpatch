...
from scripts.evaluate_fixes import (              
    BUCKET_ORDER,
    extract_fix_id_from_sample_id,
    load_jsonl_test_samples,
    parse_ground_truth_code,
    extract_types_from_fix_prompt_input,
    _bucket_primary,
)
...
def _verify_fixed_contract(
    *,
    fixed_contract_src: str,
    slither_mgr: SlitherManager,
    mythril_mgr: MythrilManager,
    enable_mythril_check: bool,
    mythril_timeout: int,
    mythril_severities: Optional[List[str]],
    mythril_uncertain_as_pass: bool,
...
\
    ...
    try:
                            ...
                            for res in det.results:
                                ...
                                if ("high" in sev_s) or ("medium" in sev_s):
                                    ...
                                    check_name = getattr(
                                        res,
                                        "check",
                                        det.ARGUMENT if hasattr(det, "ARGUMENT") else "UnknownCheck",
                                    )
        ...
        if enable_mythril_check:
            try:
                mr = mythril_mgr.analyze_contract(
                    tmp,
                    timeout=int(mythril_timeout),
                    severities=mythril_severities if mythril_severities is not None else ("high", "medium"),
                    max_issues=50,
                )
...
_FUNC_BLOCK_RE = re.compile(r"\bfunction\b", re.MULTILINE)
...
\
    ...
    if needs_erc20 and "interface IERC20" not in s:
        ...
        m = re.search(r"^\s*pragma\s+solidity\b[^\n]*\n", s, flags=re.MULTILINE)
        if m:
            insert_at = m.end()
    ...
    if needs_safemath and "using SafeMath for uint256" not in s:
        s = re.sub(
            r"(\bcontract\s+[A-Za-z_][A-Za-z0-9_]*\s*\{\s*\n)",
            r"\1    using SafeMath for uint256;\n",
            s,
            count=1,
            flags=re.MULTILINE,
        )
    ...
    user_type_decl = re.compile(r"^\s*([A-Z][A-Za-z0-9_]*)\b(.*);?\s*$")
    for line in lines:
        ...
        if m:
            ...
            if re.match(r"^\s*(contract|interface|library)\b", line):
                out_lines.append(line)
                continue
                                                                  
            if re.search(r"\b(memory|storage|calldata)\b", rest):
                out_lines.append(line)
                continue
            token_like = bool(re.search(r"\btoken\b", rest, flags=re.IGNORECASE) or tname.lower().endswith("token"))
            has_visibility = bool(re.search(r"\b(public|private|internal|external)\b", rest))
    ...
    try:
        defined = set()
        for _m in re.finditer(r"\b(struct|contract|interface|library|enum)\s+([A-Za-z_][A-Za-z0-9_]*)\b", s2):
            defined.add(_m.group(2))

        decls = list(
            re.finditer(r"\b([A-Z][A-Za-z0-9_]*)\s+(memory|storage|calldata)\s+([A-Za-z_][A-Za-z0-9_]*)\s*;", s2)
        )
        ...
        for dm in decls:
            ...
            fields = sorted(set(re.findall(rf"\b{re.escape(vname)}\.([A-Za-z_][A-Za-z0-9_]*)\b", s2)))
            if not fields:
                fields = ["_dummy"]
        ...
        if struct_defs:
            ...
            pat = r"(\bcontract\s+[A-Za-z_][A-Za-z0-9_]*\s*\{\s*\n(?:\s*using\s+SafeMath\s+for\s+uint256;\s*\n)?)"
            s2 = re.sub(pat, r"\1" + blob, s2, count=1, flags=re.MULTILINE)
    except Exception:
        pass
...
\
    ...
    return "\n".join(
        [
            ...
            "",
            "# High Risk Findings",
            f"## [[H-01] {name}](https://example.com)",
            desc,
            "Recommend",
            "Apply minimal necessary changes to fix the vulnerability while preserving intended behavior.",
            "",
...
\
    ...
    mythril_mgr = MythrilManager(
        debug=bool(getattr(args, "verbose", False)),
    ...
    for sample_id, fix_id, sample in iterable:
        ...
        with tempfile.TemporaryDirectory(prefix="contracttinker_eval_") as td:
            ...
            report_md.write_text(
                _synthesize_audit_report_md(sample_id=sample_id, prompt_input=str(sample.get("input", "") or "")),
                encoding="utf-8",
            )
            ...
            try:
                ...
                repair = Repair(
                    ...
                    str(proj_dir) + os.sep,
                    solc_remaps="",
                    model_name=str(getattr(args, "model", "gpt-3.5-turbo") or "gpt-3.5-turbo"),
                    validator_model_name=str(getattr(args, "contracttinker_validator_model", "gpt-4") or "gpt-4"),
                    parsed_report_dir=str(td_path),
                    enable_call_graph=bool(getattr(args, "contracttinker_enable_call_graph", False)),
                )
        ...
        if fixed_func:
            ...
            ver, _ = _verify_fixed_contract(
                fixed_contract_src=fixed_contract_src,
                slither_mgr=slither_mgr,
                mythril_mgr=mythril_mgr,
        ...
        result: Dict[str, Any] = {
            "sample_id": sample_id,
            "contract": func.contract_path,
            "function": func.function_name,
            "total_generated": 1,
            "correct_generated": 1 if is_correct else 0,
            "best_candidate_code": fixed_func,
            "passed_candidate_codes": [fixed_func] if (is_correct and fixed_func) else [],
            "sample_time_sec": float(gen_dt + verify_dt),
            "sample_gen_time_sec": float(gen_dt),
            "sample_verify_time_sec": float(verify_dt),
            "llm_total_tokens": 0,
            "llm_prompt_tokens": 0,
            "llm_completion_tokens": 0,
            "llm_requests": 0,
            "verification": ver,
            "contracttinker_error": patch_error,
            "contracttinker_raw_output": _truncate(raw_patch_output or "", 1800) if raw_patch_output else None,
            "vulnerability_types": parsed_types,
            "vuln_bucket": vuln_bucket,
        }
        if gt_code:
    ...
    metrics["sample_verify_time_sec_avg"] = (
        sum(float(r.get("sample_verify_time_sec", 0.0) or 0.0) for r in eval_results) / n_eval
    )
...
def main():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Path to LLM4SMAPR/ContractTinker directory (defaults to repo_root/LLM4SMAPR/ContractTinker)",
    )
    p.add_argument(
        "--contracttinker-enable-call-graph",
        action="store_true",
        help="Enable ContractTinker call-graph generation via slither CLI + dot parsing (requires extra deps).",
    )
