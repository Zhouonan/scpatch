...
def extract_fix_id_from_sample_id(sample_id: str) -> Optional[int]:
    m = re.search(r"fix_(\d+)", str(sample_id))
    if m:
...
def _suspicious_flags(original_code: str, fixed_code: str) -> List[str]:
    ...
    if re.search(r"\brevert\s*\(", fl) or re.search(r"\bthrow\b", fl) or "require(false" in fl:
...
def main():
    ...
    for d in details:
        ...
        row = {
            "sample_id": sample_id,
            "fix_id": fix_id,
            "function": d.get("function") or (getattr(func, "function_name", None) if func is not None else None),
            "contract": d.get("contract") or (getattr(func, "contract_path", None) if func is not None else None),
            "vuln_bucket": d.get("vuln_bucket"),
            ...
            "flags": flags,
            "original_function_code": _safe_str(original_code),
            "best_candidate_code": _safe_str(fixed_best),
            "passed_candidate_codes": d.get("passed_candidate_codes") if isinstance(d.get("passed_candidate_codes"), list) else [],
        }
        rows.append(row)
    ...
    for r in rows:
        ...
        page.append("\n## Diff (original -> best)\n\n")
        diff = _unified_diff(
            r.get("original_function_code", ""),
            r.get("best_candidate_code", ""),
            fromfile="original",
            tofile="best_candidate",
        )
        page.append("```diff\n")
