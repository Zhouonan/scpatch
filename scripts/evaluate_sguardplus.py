...
_FUNC_DECL_RE = re.compile(r"\bfunction\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(")
...
def _run_sguardplus_on_contract(
    *,
    contract_file: str,
    sguard_dir: str,
    node_bin: str,
    ...
    with tempfile.TemporaryDirectory(prefix="sguardplus_") as td:
        ...
        if not os.path.exists(fixed_path):
            ...
            if vul_report is None:
                vul_report = {
                    "_sguard_rc": out.returncode,
                    "_sguard_stdout": _truncate(out.stdout or "", 1200),
                    "_sguard_stderr": _truncate(out.stderr or "", 1200),
                }
...
\
    ...
    with tempfile.TemporaryDirectory(prefix="sguardplus_src_") as td:
        td_path = Path(td)
        safe_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", filename_hint or "contract.sol")
        tmp_contract = td_path / safe_name
        tmp_contract.write_text(contract_src or "", encoding="utf-8")
        return _run_sguardplus_on_contract(
            contract_file=str(tmp_contract),
            sguard_dir=sguard_dir,
            node_bin=node_bin,
...
\
    ...
    for sample_id, fix_id, sample in sample_infos:
        ...
        row = {
            "sample_id": sample_id,
            "fix_id": fix_id,
            "contract_file": contract_key,
            "original_contract_file": contract_path,
            "contract_name": contract_name,
            "function_name": func.function_name,
            "original_function_code": func.function_code,
            "original_solidity_version": func.solidity_version,
            "jsonl_sample": sample,
            "reference_code": gt_code if gt_code else None,
            "simplified_contract_src": simplified_src,
        }
    ...
    for contract_key, rows in it_contracts:
        ...
        if use_simplified:
            ...
            for r in rows:
                if r.get("simplified_contract_src"):
                    ...
                    break
            if src:
                fixed_src, vul_report, gen_time_sec = _run_sguardplus_on_source(
                    contract_src=str(src),
                    filename_hint=f"{contract_key}.sol",
                    sguard_dir=str(getattr(args, "sguard_dir")),
                    node_bin=str(getattr(args, "node_bin", "node") or "node"),
                    python_bin=str(getattr(args, "python_bin", "python") or "python"),
                    timeout=int(getattr(args, "sguard_timeout", 600) or 600),
                )
        else:
            fixed_src, vul_report, gen_time_sec = _run_sguardplus_on_contract(
                contract_file=str(contract_key),
                sguard_dir=str(getattr(args, "sguard_dir")),
                node_bin=str(getattr(args, "node_bin", "node") or "node"),
                python_bin=str(getattr(args, "python_bin", "python") or "python"),
                timeout=int(getattr(args, "sguard_timeout", 600) or 600),
        ...
        contract_cache[contract_key] = {
            "fixed_contract_src": fixed_src,
            "vul_report": vul_report,
            "supported_funcs": supported_funcs,
            "gen_time_sec": gen_time_sec,
            "verify_time_sec": verify_time_sec,
    ...
    for row in it_samples:
        ...
        result: Dict[str, Any] = {
            ...
            "contract": row.get("original_contract_file") or row.get("contract_file"),
            "function": row.get("function_name"),
            "total_generated": 1,
            "correct_generated": 1 if is_correct else 0,
            "best_candidate_code": best_code,
            "passed_candidate_codes": [best_code] if (is_correct and best_code) else [],
            "sample_time_sec": float(sample_gen_time + sample_verify_time),
            "sample_gen_time_sec": float(sample_gen_time),
            "sample_verify_time_sec": float(sample_verify_time),
            "llm_total_tokens": 0,
            "llm_prompt_tokens": 0,
            "llm_completion_tokens": 0,
            "llm_requests": 0,
            "sguard_supported": bool(is_supported),
            "sguard_contract_verified": bool(is_correct),
            "verification": ver,                                                                         
            "sguard_use_simplified_contract": bool(use_simplified),
    ...
    metrics["sguard_supported_rate"] = (
        sum(1 for r in eval_results if bool(r.get("sguard_supported"))) / n_eval
    )
...
def main():
    ...
    p.add_argument(
        "--use-simplified-contract",
        action="store_true",
        help="用 DB 中的 function_context 构造一个可编译的简化合约作为 sGuardPlus 输入（推荐用于函数级评测）。",
    )
