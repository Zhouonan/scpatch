...
def main() -> int:
    parser = argparse.ArgumentParser(
        description='Remove null-valued fields inside the top-level "fix" object.'
    )
    parser.add_argument(
        "--input",
        required=True,
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="If set, writes cleaned files to this directory (mirrors filenames).",
    )
    ...
    print(
        f"done files_total={len(files)} changed={changed_files} "
        f"skipped={skipped} removed_fix_keys={total_removed_keys}"
    )
    return 0
