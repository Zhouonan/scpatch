...
def _compute_bleu_for_details(details: List[Dict[str, Any]]) -> Tuple[List[float], List[Dict[str, Any]]]:
    ...
    if not NLTK_AVAILABLE:
        raise RuntimeError(
            "NLTK is not available. Install it (and its deps) first, e.g.\n"
            "  pip install nltk\n"
        ...
        rows.append(
            {
                ...
                "correct_generated": d.get("correct_generated", ""),
                "total_generated": d.get("total_generated", ""),
                "bleu": bleu,
                "ref_chars": _safe_len(ref),
                "cand_chars": _safe_len(cand),
            }
        )
...
def _summarize_bleu(bleu_scores: List[float]) -> Dict[str, float]:
    ...
    return {
        "count": float(len(bleu_scores)),
        "mean": float(mean(bleu_scores)),
        "median": float(median(bleu_scores)),
        "p10": float(_percentile(s, 10)),
        "p25": float(_percentile(s, 25)),
        "p75": float(_percentile(s, 75)),
...
def _print_file_report(
    path: Path,
    stored_bleu: Optional[float],
    recomputed_bleu: Optional[float],
    counts: Dict[str, int],
    ...
    if compact:
        ...
        print(
            f"{path.name}\tstored={stored_str}\trecomputed={recomputed_str}\tabs_diff={diff_str}\t"
            f"with_pair={counts['with_pair']}/{counts['details']}\t"
            f"mean={summary['mean']:.4f}\tmed={summary['median']:.4f}\tp10={summary['p10']:.4f}\tp90={summary['p90']:.4f}"
        )
        return
    ...
    print(
        "counts: "
        f"details={counts['details']}, "
        f"with_pair={counts['with_pair']}, "
        f"missing_ref={counts['missing_ref']}, "
        f"missing_cand={counts['missing_cand']}"
    )
    print(
        "bleu stats: "
    ...
    if show_examples > 0 and rows:
        ...
        for r in low:
            print(
                f"- sample_id={r['sample_id']} bleu={float(r['bleu']):.6f} "
                f"ref_chars={r['ref_chars']} cand_chars={r['cand_chars']} func={r['function']}"
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--input_dir",
        type=str,
        default="results/system_eval/qwen_sft_1216",
    ...
    parser.add_argument(
        ...
        help="Filter samples before BLEU computation by correct_generated.",
    )
    parser.add_argument(
        "--compact",
        action="store_true",
        help="Print one TSV line per file (no top/bottom sample lists).",
    )
    parser.add_argument(
        "--aggregate",
        action="store_true",
        help="Also print an overall summary aggregated across all matched files.",
    )
    ...
    if args.compact:
        print(
            "file\tstored\trecomputed\tabs_diff\twith_pair\tmean\tmed\tp10\tp90"
        )
    ...
    if args.aggregate:
        ...
        print(
            f"pairs={int(overall['count'])} mean={overall['mean']:.6f} median={overall['median']:.6f} "
            f"p10={overall['p10']:.6f} p90={overall['p90']:.6f} min={overall['min']:.6f} max={overall['max']:.6f}"
        )
