...
def strip_leading_noncode_lines(text: str) -> str:
    ...
    def is_code_like(s: str) -> bool:
        ...
        if s.startswith(("//", "/*", "*", "require", "if", "for", "while", "return", "emit", "assert", 
                         "revert", "{", "}", "_")):
            return True
        if s.endswith(("{", "}", ";")):
            return True
...
def main():
    ...
    tok = run_staggered_load(
        accelerator,
        enabled=bool(args.stagger_load),
        group_size=int(args.stagger_load_group_size),
        sleep_s=float(args.stagger_load_sleep),
        fn=lambda: AutoTokenizer.from_pretrained(args.base_model, trust_remote_code=True),
    )
    ...
    policy_base = run_staggered_load(
        accelerator,
        enabled=bool(args.stagger_load),
        group_size=int(args.stagger_load_group_size),
        sleep_s=float(args.stagger_load_sleep),
    ...
    policy = run_staggered_load(
        accelerator,
        enabled=bool(args.stagger_load),
        group_size=int(args.stagger_load_group_size),
        sleep_s=float(args.stagger_load_sleep),
        fn=lambda: PeftModel.from_pretrained(policy_base, args.sft_lora, is_trainable=True),
    )
    policy.train()
    if args.gradient_checkpointing:
    ...
    ref_base = run_staggered_load(
        ...
        fn=lambda: AutoModelForCausalLM.from_pretrained(
            args.base_model,
            trust_remote_code=True,
            torch_dtype=(torch.float32 if ref_device == "cpu" else dtype),
            device_map=None,
            low_cpu_mem_usage=True,
        ),
    )
    ref_base.to(ref_device)
    ref = run_staggered_load(
        accelerator,
        enabled=bool(args.stagger_load),
        group_size=int(args.stagger_load_group_size),
        sleep_s=float(args.stagger_load_sleep),
        fn=lambda: PeftModel.from_pretrained(ref_base, args.sft_lora).to(ref_device),
    )
    ref.eval()
                ...
                with torch.no_grad():
                    ...
                    while remaining > 0:
                        ...
                        out = gen_model.generate(
                            input_ids=b_input_ids, attention_mask=b_attn,
            ...
            if accelerator.is_main_process:
                ...
                if log_f:
                    log_f.write(json.dumps({
                        "step": step, "reward": avg_reward, "cost": g_cost, "lambda": lagrange_lambda, "loss": total_loss
                    }) + "\n")
                    log_f.flush()
