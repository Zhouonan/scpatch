...
_RE_CODE_FENCE = re.compile(r"```(?:[a-zA-Z0-9_-]+)?\s*(.*?)\s*```", re.DOTALL | re.IGNORECASE)
...
def strip_leading_noncode_lines(text: str) -> str:
    ...
    def is_code_like(s: str) -> bool:
        ...
        if s.endswith(("{", "}", ";")):
            return True
        if re.match(r"^(uint|int|bool|address|bytes|string|mapping)\b", s):
            return True
...
def extract_fix_id_from_sample_id(sample_id: str) -> Optional[int]:
    m = re.search(r"fix_(\d+)", sample_id or "")
    if m:
...
def extract_function_name(inp: str) -> Optional[str]:
    m = re.search(r"\*\*Function:\*\*\s*([A-Za-z_]\w*)", inp)
    if m:
        return m.group(1).strip()
    return None
...
def mythril_check(
    ...
    contract_src: str,
    timeout: int = 120,
    severities: Optional[Iterable[str]] = ("high", "medium"),
    max_issues: int = 50,
    verbose: bool = False,
    debug: bool = False,
    debug_save_dir: Optional[Path] = None,
    ...
    res = mythril_mgr.analyze_source(
        contract_src,
        timeout=timeout,
...
def _make_groups(
    world_size: int, num_policy_ranks: int, *, ref_mode: str, rank: int
) -> Tuple[Optional[dist.ProcessGroup], Optional[dist.ProcessGroup], Optional[dist.ProcessGroup]]:
\
\
...
def paired_ref_request(
    *,
    peer_rank: int,
    input_ids: torch.Tensor,
    attention_mask: torch.Tensor,
    gen_ids: torch.Tensor,
    pair_group: Optional[dist.ProcessGroup] = None,
) -> torch.Tensor:
\
\
\
       
    out = paired_ref_request_batch(
        peer_rank=peer_rank,
        input_ids=input_ids,
        attention_mask=attention_mask,
        gen_ids_batch=gen_ids,
        pair_group=pair_group,
    )
    return out.view(1)


def paired_ref_request_batch(
    *,
    peer_rank: int,
    input_ids: torch.Tensor,
    attention_mask: torch.Tensor,
    gen_ids_batch: torch.Tensor,
    pair_group: Optional[dist.ProcessGroup] = None,
) -> torch.Tensor:
\
\
...
def paired_ref_worker_loop(
    ref_model: torch.nn.Module, device: torch.device, *, pair_group: Optional[dist.ProcessGroup]
) -> None:
\
\
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="If set, candidates with text similarity(original_func, new_func) >= threshold will be treated as non-success (and incur extra cost). Example: 0.995",
    )
    parser.add_argument(
        "--orig-sim-cost",
        type=float,
        default=1.0,
        help="Extra constraint cost added when a candidate is too similar to the original (only when --max-orig-sim is set).",
    )
    ...
    parser.add_argument(
        "--profile-sync",
        action="store_true",
        help="Add torch.cuda.synchronize() around timed regions for more accurate GPU timings (slower).",
    )
    parser.add_argument(
        "--debug-dist",
        action="store_true",
        help="Print rank-local progress markers (send/recv/barrier) with timestamps to debug hangs.",
    )
    parser.add_argument(
        "--no-step-barrier",
        action="store_true",
        help="Disable any explicit per-step barrier. Recommended for stability; DDP backward already synchronizes grads.",
    )
    parser.add_argument(
        "--no-final-barrier",
        action="store_true",
        help="Disable final global barrier on exit (prevents hangs if some ranks crash/hang).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="local",
        choices=["local", "paired"],
        help="local: ref runs in the same process/device as policy (world_size=1 recommended); paired: dedicate ranks for ref inference (world_size=2/4).",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="off",
        choices=["off", "always", "gate"],
        help="RAG usage mode: off=disable; always=always use RAG-augmented prompt; gate=train a small RL gate to decide per-sample.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="If set (or if rag-index-path missing), build RAG corpus from this fix_sft jsonl (expects input/output fields).",
    )
    ...
    parser.add_argument("--rag-gate-lr", type=float, default=5e-4, help="Learning rate for the RAG gate policy.")
    ...
    parser.add_argument(
        "--verbose-dump-per-step",
        type=int,
        default=0,
        help="When --verbose is set, dump up to N samples per step: prompt, reference output, and model candidates (rank0 only).",
    )
    ...
    if args.ref_mode == "local":
        ...
        ref_base = AutoModelForCausalLM.from_pretrained(
            args.base_model,
            trust_remote_code=True,
            torch_dtype=(torch.float32 if ref_local_device == "cpu" else dtype),
            device_map=None,
            low_cpu_mem_usage=True,
        )
    ...
    try:
        for step in range(1, args.steps + 1):
            ...
            for item in batch_items:
                ...
                if args.use_db_contract and db_session:
                    ...
                    if fix_id is not None:
                        if fix_id not in db_cache:
                            db_cache[fix_id] = (
                                db_session.query(SmartContractFunction)
                                .filter(SmartContractFunction.id == fix_id)
                                .first()
                            )
                        row = db_cache.get(fix_id)
                        if row:
                            func_data_for_validation = {
                                "function_name": row.function_name,
                                "function_code": row.function_code,
                                "solidity_version": row.solidity_version or "0.8.0",
                ...
                if args.ref_mode == "paired":
                    ...
                    for s in range(0, len(cand_ids), mb):
                        ...
                        lp_b = paired_ref_request_batch(
                            peer_rank=int(peer_ref_rank),
                            input_ids=input_ids,
                            attention_mask=attn,
                            gen_ids_batch=gen_b,
                            pair_group=pg_pair,
                    ...
                    for s in range(0, len(cand_ids), mb):
                        ...
                        gen_b = torch.cat([t.to(device) for t in cand_ids[s:e]], dim=0)          
                        B = int(gen_b.shape[0])
                        lp_b = compute_sequence_logprob(
                            ref_local,
                            input_ids.to(ref_local_device).expand(B, -1).contiguous(),
                            gen_b.to(ref_local_device),
                            attn.to(ref_local_device).expand(B, -1).contiguous(),
                        ).to(device=device)       
                        for i in range(lp_b.shape[0]):
                            ref_lps.append(lp_b[i : i + 1].to(device))
                    _maybe_cuda_sync(device, bool(args.profile and args.profile_sync))
                ...
                for cand_i, (gen_ids, gen_txt) in enumerate(zip(cand_ids, cand_texts)):
                    ...
                    if not fixed_func:
                        ...
                        if func_data_for_validation and slice_builder:
                            ...
                            repaired = rebuilt if rebuilt else slice_builder.build_simplified_contract(
                                func_data_for_validation, fixed_code=fixed_func
                            )
                        elif contract:
                            ...
                            if vkey in verify_cache:
                                ...
                                if not ok:
                                    ...
                                    if mythril_mgr:
                                        ...
                                        mp, mic, missues, merr = mythril_check(
                                            mythril_mgr,
                                            repaired,
                                            timeout=args.mythril_timeout,
                                            severities=mythril_severities,
                                            debug=args.debug_verify,
                                            debug_save_dir=debug_save_dir,
                                            tag=cand_tag,
                                        )
                    ...
                    tt.start()
                    policy_out = policy(
                        input_ids=torch.cat([input_ids, gen_ids_on_policy], dim=1),
                        attention_mask=torch.cat([attn, torch.ones_like(gen_ids_on_policy)], dim=1),
                        use_cache=False,
                    )
                    L = input_ids.shape[1]
            ...
            if world_size > 1 and pg_policy is not None and num_policy >= 1:
                t = torch.tensor(
                    [local_reward_sum, local_succ_sum, local_cost_sum, local_count],
                    device=device,
                    dtype=torch.float32,
                )
            ...
            if rank == 0:
                print(
                    f"[step {step}] reward={avg_reward:.3f} cost={g_cost:.3f} succ={succ_rate:.2%} lambda={lagrange_lambda:.3f}"
                )
                if log_f:
                    log_f.write(
                        json.dumps({"step": step, "reward": avg_reward, "cost": g_cost, "lambda": lagrange_lambda, "loss": total_loss})
                        + "\n"
                    )
                    log_f.flush()
                ...
                if world_size > 1 and pg_policy is not None and num_policy >= 1:
                    t = torch.tensor(
                        [
                            ...
                            step_t["backward_s"],
                            step_t["opt_s"],
                            float(step_counts["cands"]),
                            float(step_counts["verified"]),
                            float(step_counts["compile_calls"]),
                            float(step_counts["slither_calls"]),
                            float(step_counts["mythril_calls"]),
                        ],
                        device=device,
                        dtype=torch.float32,
                    )
                        ...
                        json.dumps(
                            {
                                ...
                                "policy_ranks": num_policy,
                                "ref_mode": args.ref_mode,
                                "k": int(args.k),
                                "gen_microbatch": int(getattr(args, "gen_microbatch", 1)),
                                "batch_prompts": int(args.batch_prompts),
                                "timing_s": {
                                    "tokenize": tokenize_s,
                                    "generate": gen_s,
                                    "verify": verify_s,
                                    "ref_logprob": ref_lp_s,
                                    "policy_logprob": pol_lp_s,
                                    "backward": backward_s,
                                    "opt": opt_s,
                                },
                                "counts": {
