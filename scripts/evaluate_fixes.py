...
def _bucket_primary(display_types) -> str:
    ...
    if mapped:
        return mapped[0]
    return 'OTHER'
_TYPE_LINE_RE = re.compile('^\\s*-\\s*Type\\s*:\\s*(.+?)\\s*$', re.IGNORECASE)
...
def parse_ground_truth_code(output: str) -> str:
    code_match = re.search('```(?:solidity)?\\s*(.*?)\\s*```', output, re.DOTALL)
    if code_match:
        return code_match.group(1).strip()
    return output.strip()

def extract_fix_id_from_sample_id(sample_id: str) -> int:
    match = re.search('fix_(\\d+)', sample_id)
    if match:
...
def _eval_one_sample(task: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if baseline_only:
        ...
        result: Dict[str, Any] = {'sample_id': task.get('sample_id'), 'contract': func_data.get('contract_path'), 'function': func_data.get('function_name'), 'total_generated': 1, 'correct_generated': 1 if is_ok else 0, 'best_candidate_code': orig_code if is_ok else None, 'passed_candidate_codes': [orig_code] if is_ok else [], 'best_candidate_ir_sim': float(baseline_ver.get('ir_sim', 0.0) or 0.0), 'best_candidate_ir_change_rate': float(baseline_ver.get('ir_change_rate', 0.0) or 0.0), 'passed_candidate_ir_sims': [float(baseline_ver.get('ir_sim', 0.0) or 0.0)] if is_ok else [], 'passed_candidate_ir_change_rates': [float(baseline_ver.get('ir_change_rate', 0.0) or 0.0)] if is_ok else [], 'best_candidate_orig_sim': 1.0 if is_ok else None, 'passed_candidate_orig_sims': [1.0] if is_ok else [], 'sample_time_sec': dt, 'sample_gen_time_sec': 0.0, 'sample_verify_time_sec': dt, 'llm_total_tokens': 0, 'llm_prompt_tokens': 0, 'llm_completion_tokens': 0, 'llm_requests': 0, 'rag': {'rag_used': False}, 'baseline_verification': baseline_ver}
    ...
    gen_t0 = time.time()
    candidates = _EVAL_FIXER.generate_fix_candidates(func_data, annotation, n=n_samples, prompt_override=prompt_override, system_prompt_override=system_prompt_override, verify=False, sample_id=str(task.get('sample_id')) if task.get('sample_id') is not None else None)
    ...
    result: Dict[str, Any] = {'sample_id': task.get('sample_id'), 'contract': func_data.get('contract_path'), 'function': func_data.get('function_name'), 'total_generated': n_samples, 'correct_generated': correct_count, 'best_candidate_code': best_code, 'passed_candidate_codes': passed_codes, 'best_candidate_ir_sim': best_ir_sim, 'best_candidate_ir_change_rate': best_ir_change_rate, 'passed_candidate_ir_sims': passed_ir_sims, 'passed_candidate_ir_change_rates': passed_ir_change_rates, 'best_candidate_orig_sim': best_orig_sim, 'passed_candidate_orig_sims': passed_orig_sims, 'sample_time_sec': dt, 'sample_gen_time_sec': gen_dt, 'sample_verify_time_sec': verify_dt, 'llm_total_tokens': max(0, tok1 - tok0), 'llm_prompt_tokens': max(0, pt1 - pt0), 'llm_completion_tokens': max(0, ct1 - ct0), 'llm_requests': max(0, req1 - req0), 'rag': rag_meta_from_candidate}
    if record_baseline:
...
def run_evaluation(args: argparse.Namespace) -> dict:
    ...
    config = FixerConfig(api_key=api_key, base_url=args.base_url, model=args.model, temperature=args.temperature, top_p=args.top_p, max_tokens=args.max_tokens, seed=args.seed, verbose=args.verbose, print_llm_responses=bool(getattr(args, 'print_llm_responses', False)), llm_responses_out=getattr(args, 'llm_responses_out', None), reasoning_effort=getattr(args, 'reasoning_effort', None), max_output_tokens=getattr(args, 'max_output_tokens', None), force_single_n=bool(getattr(args, 'force_single_n', False)), llm_request_workers=int(getattr(args, 'llm_request_workers', 1) or 1), enable_compilation_check=True, enable_slither_check=True, enable_mythril_check=bool(getattr(args, 'enable_mythril_check', False)), mythril_timeout=int(getattr(args, 'mythril_timeout', 120) or 120), mythril_severities=_parse_csv_strs(getattr(args, 'mythril_severities', None)), mythril_bin=str(getattr(args, 'mythril_bin', 'myth') or 'myth'), evaluation_mode=True, strict_verification=bool(getattr(args, 'strict_verification', False)), mythril_uncertain_as_pass=bool(getattr(args, 'mythril_uncertain_as_pass', True)), rag_mode=str(getattr(args, 'rag_mode', 'off') or 'off'), rag_index_path=getattr(args, 'rag_index_path', None), rag_build_from_jsonl=getattr(args, 'rag_build_from_jsonl', None), rag_build_limit=int(getattr(args, 'rag_build_limit', 5000) or 5000), rag_top_k=int(getattr(args, 'rag_top_k', 3) or 3), rag_max_demos=int(getattr(args, 'rag_max_demos', 3) or 3), rag_max_chars_each=int(getattr(args, 'rag_max_chars_each', 800) or 800), rag_max_added_tokens=getattr(args, 'rag_max_added_tokens', None), rag_mmr_lambda=float(getattr(args, 'rag_mmr_lambda', 0.7) or 0.7), rag_fusion_weights=str(getattr(args, 'rag_fusion_weights', '0.35,0.35,0.2,0.1') or '0.35,0.35,0.2,0.1'))
    ...
    if args.jsonl_path:
        ...
        for (sample_id, fix_id, sample) in sample_infos:
            ...
            eval_data.append({'func': func, 'sample_id': sample_id, 'ground_truth_code': gt_code, 'jsonl_sample': sample, 'prompt_override': prompt_override, 'system_prompt_override': system_prompt_override})
        print(f'成功加载 {len(eval_data)} 个样本（关联数据库后）')
    else:
        query = session.query(SmartContractFunction).filter(SmartContractFunction.llm_audit.isnot(None))
        all_funcs = query.all()
        vulnerable_funcs = [f for f in all_funcs if f.llm_audit and f.llm_audit.get('label') == 'vulnerable']
        if args.limit:
            vulnerable_funcs = vulnerable_funcs[:args.limit]
        print(f'评估数据集大小: {len(vulnerable_funcs)} (从数据库加载)')
        eval_data = [{'func': f, 'sample_id': f.id, 'ground_truth_code': None, 'jsonl_sample': None} for f in vulnerable_funcs]
    tasks: List[Dict[str, Any]] = []
    for data_item in eval_data:
        func = data_item['func']
        func_data = {'function_name': func.function_name, 'function_code': func.function_code, 'solidity_version': func.solidity_version, 'contract_context': func.contract_context, 'contract_path': func.contract_path}
        tasks.append({'sample_id': data_item['sample_id'], 'func_data': func_data, 'annotation': func.llm_audit, 'ground_truth_code': data_item.get('ground_truth_code'), 'jsonl_sample': data_item.get('jsonl_sample'), 'n_samples': int(args.n_samples), 'max_ir_change_rate': getattr(args, 'max_ir_change_rate', None), 'max_orig_sim': getattr(args, 'max_orig_sim', None), 'prompt_override': data_item.get('prompt_override'), 'system_prompt_override': data_item.get('system_prompt_override'), 'include_source': bool(getattr(args, 'include_source', False)), 'record_baseline': bool(getattr(args, 'record_baseline', False)), 'baseline_only': bool(getattr(args, 'baseline_only', False))})
    ...
    if sample_workers <= 1 or not tasks:
        for task in tqdm(tasks, desc='Evaluating', disable=disable_tqdm):
            ...
            if baseline_only:
                ...
                result: Dict[str, Any] = {'sample_id': task.get('sample_id'), 'contract': func_data.get('contract_path'), 'function': func_data.get('function_name'), 'total_generated': 1, 'correct_generated': 1 if is_ok else 0, 'best_candidate_code': orig_code if is_ok else None, 'passed_candidate_codes': [orig_code] if is_ok else [], 'best_candidate_ir_sim': float(baseline_ver.get('ir_sim', 0.0) or 0.0), 'best_candidate_ir_change_rate': float(baseline_ver.get('ir_change_rate', 0.0) or 0.0), 'passed_candidate_ir_sims': [float(baseline_ver.get('ir_sim', 0.0) or 0.0)] if is_ok else [], 'passed_candidate_ir_change_rates': [float(baseline_ver.get('ir_change_rate', 0.0) or 0.0)] if is_ok else [], 'sample_time_sec': dtb, 'sample_gen_time_sec': 0.0, 'sample_verify_time_sec': dtb, 'llm_total_tokens': 0, 'llm_prompt_tokens': 0, 'llm_completion_tokens': 0, 'llm_requests': 0, 'rag': {'rag_used': False}, 'baseline_verification': baseline_ver}
                parsed_types = []
                if isinstance(jsonl_sample, dict):
                    parsed_types = extract_types_from_fix_prompt_input(jsonl_sample.get('input', ''))
                result_types = parsed_types
            ...
            gen_t0 = time.time()
            candidates = fixer.generate_fix_candidates(func_data, annotation, n=args.n_samples, prompt_override=task.get('prompt_override'), system_prompt_override=task.get('system_prompt_override'), verify=False, sample_id=str(task.get('sample_id')) if task.get('sample_id') is not None else None)
            gen_dt = float(time.time() - gen_t0)
            verify_dt = 0.0
            if isinstance(candidates, list) and candidates:
                for cand in candidates:
            ...
            for cand in candidates:
                ...
                if is_correct:
                    ...
                    if best_code is None:
                        ...
                        best_ir_sim = float(ver.get('ir_sim', 0.0) or 0.0)
                        best_ir_change_rate = float(ver.get('ir_change_rate', 0.0) or 0.0)
            result: Dict[str, Any] = {'sample_id': task.get('sample_id'), 'contract': func_data.get('contract_path'), 'function': func_data.get('function_name'), 'total_generated': int(args.n_samples), 'correct_generated': correct_count, 'best_candidate_code': best_code, 'passed_candidate_codes': passed_codes, 'best_candidate_ir_sim': best_ir_sim, 'best_candidate_ir_change_rate': best_ir_change_rate, 'passed_candidate_ir_sims': passed_ir_sims, 'passed_candidate_ir_change_rates': passed_ir_change_rates, 'sample_time_sec': dt, 'sample_gen_time_sec': gen_dt, 'sample_verify_time_sec': verify_dt, 'llm_total_tokens': max(0, tok1 - tok0), 'llm_prompt_tokens': max(0, pt1 - pt0), 'llm_completion_tokens': max(0, ct1 - ct0), 'llm_requests': max(0, req1 - req0), 'rag': rag_meta_from_candidate}
            if record_baseline:
                result['baseline_verification'] = baseline_ver
            if bool(getattr(args, 'include_source', False)):
                result['original_function_code'] = func_data.get('function_code')
        ...
        fixer_cfg_kwargs = {'api_key': config.api_key, 'base_url': config.base_url, 'model': config.model, 'temperature': config.temperature, 'top_p': config.top_p, 'max_tokens': config.max_tokens, 'seed': config.seed, 'verbose': False, 'print_llm_responses': bool(getattr(args, 'print_llm_responses', False)), 'llm_responses_out': getattr(args, 'llm_responses_out', None), 'llm_responses_append_pid': True, 'reasoning_effort': getattr(args, 'reasoning_effort', None), 'max_output_tokens': getattr(args, 'max_output_tokens', None), 'force_single_n': bool(getattr(config, 'force_single_n', False)), 'llm_request_workers': int(getattr(config, 'llm_request_workers', 1) or 1), 'enable_compilation_check': config.enable_compilation_check, 'enable_slither_check': config.enable_slither_check, 'enable_mythril_check': config.enable_mythril_check, 'evaluation_mode': config.evaluation_mode, 'timeout': config.timeout, 'max_retries': config.max_retries, 'retry_delay': config.retry_delay, 'presence_penalty': config.presence_penalty, 'frequency_penalty': config.frequency_penalty, 'use_rich_fix_prompt': config.use_rich_fix_prompt, 'enable_error_feedback': config.enable_error_feedback, 'max_error_lines': config.max_error_lines, 'mythril_timeout': config.mythril_timeout, 'mythril_severities': config.mythril_severities, 'mythril_bin': config.mythril_bin, 'strict_verification': bool(getattr(config, 'strict_verification', False)), 'rag_mode': str(getattr(config, 'rag_mode', 'off') or 'off'), 'rag_index_path': getattr(config, 'rag_index_path', None), 'rag_build_from_jsonl': getattr(config, 'rag_build_from_jsonl', None), 'rag_build_limit': int(getattr(config, 'rag_build_limit', 5000) or 5000), 'rag_top_k': int(getattr(config, 'rag_top_k', 3) or 3), 'rag_max_demos': int(getattr(config, 'rag_max_demos', 3) or 3), 'rag_max_chars_each': int(getattr(config, 'rag_max_chars_each', 800) or 800), 'rag_max_added_tokens': getattr(config, 'rag_max_added_tokens', None), 'rag_mmr_lambda': float(getattr(config, 'rag_mmr_lambda', 0.7) or 0.7), 'rag_fusion_weights': str(getattr(config, 'rag_fusion_weights', '0.35,0.35,0.2,0.1') or '0.35,0.35,0.2,0.1')}
        failures: List[Dict[str, Any]] = []
        with Executor(max_workers=sample_workers, initializer=_init_eval_worker, initargs=(fixer_cfg_kwargs,)) as ex:
            ...
            for fut in it:
                ...
                try:
                    ...
                    failures.append({'sample_id': task.get('sample_id'), 'error': repr(e), 'traceback': traceback.format_exc()})
        if failures:
...
def main():
    ...
    parser.add_argument('--print-llm-responses', action='store_true', help='仅打印每次生成的 LLM 原始完整回复到控制台（不需要开启 --verbose；并行时输出可能交错）')
    parser.add_argument('--llm-responses-out', type=str, default=None, help='可选：将每个 sample 的每个 candidate 的 LLM 原始完整回复追加写入 JSONL 文件（并行时会自动按 pid 分文件以避免冲突）')
    ...
    parser.add_argument('--use-jsonl-prompt', action='store_true', help='当指定 --jsonl-path 时：使用 JSONL 的 instruction/input 作为真实提示词生成（不从 DB 重新构建 prompt）；DB 仍用于验证（编译/Slither）')
    ...
    parser.add_argument('--include-source', action='store_true', help='在 details 中附带 original_function_code/original_solidity_version，便于人工审查（会显著增大输出文件）')
    parser.add_argument('--record-baseline', action='store_true', help='可选：在评测时额外对“原始函数代码”跑一遍验证(compile/slither/mythril)，并写入 details[*].baseline_verification 便于诊断“工具是否能检出原始漏洞”。')
    ...
    parser.add_argument('--rag-mode', type=str, default='off', choices=['off', 'always'], help='RAG usage mode during evaluation: off=disable; always=prepend retrieved demos before the user prompt.')
