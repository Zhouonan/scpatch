...
from transformers import AutoTokenizer, AutoModelForCausalLM, TrainingArguments, Trainer, DataCollatorForSeq2Seq, get_linear_schedule_with_warmup
...
for _p in (_project_root, _scripts_root):
    if _p not in sys.path:
        sys.path.append(_p)
try:
    from src.training.auxiliary_losses import CopyMechanismHead, ContrastiveStructuralLoss, compute_copy_labels
except Exception:
...
def _build_run_name(args: argparse.Namespace) -> str:
    ...
    parts = [_sanitize_for_path(model_tag, 40), f'lora_r{args.lora_r}', f'a{args.lora_alpha}', f'd{args.lora_dropout}', f'lr{args.learning_rate}', f'bs{args.batch_size}', f'ga{args.gradient_accumulation}', f'ep{args.epochs}', f'q{args.quantization}', f'pf{args.prompt_format}']
...
def _run_session_key(args: argparse.Namespace) -> str:
    fields = {'model_name': getattr(args, 'model_name', None), 'model_path': getattr(args, 'model_path', None), 'train_data': getattr(args, 'train_data', None), 'val_data': getattr(args, 'val_data', None), 'output_dir_base': getattr(args, 'output_dir', None), 'run_name': getattr(args, 'run_name', None), 'auto_output_subdir': bool(getattr(args, 'auto_output_subdir', False)), 'lora_r': getattr(args, 'lora_r', None), 'lora_alpha': getattr(args, 'lora_alpha', None), 'lora_dropout': getattr(args, 'lora_dropout', None), 'epochs': getattr(args, 'epochs', None), 'batch_size': getattr(args, 'batch_size', None), 'gradient_accumulation': getattr(args, 'gradient_accumulation', None), 'learning_rate': getattr(args, 'learning_rate', None), 'warmup_steps': getattr(args, 'warmup_steps', None), 'quantization': getattr(args, 'quantization', None), 'prompt_format': getattr(args, 'prompt_format', None), 'alpha_copy': getattr(args, 'alpha_copy', None), 'beta_struct': getattr(args, 'beta_struct', None), 'struct_temperature': getattr(args, 'struct_temperature', None)}
...
class AuxLossTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        outputs = model(input_ids=inputs.get('input_ids'), attention_mask=inputs.get('attention_mask'), labels=inputs.get('labels'), output_hidden_states=self.alpha_copy > 0 or self.beta_struct > 0, return_dict=True)
        ...
        if self.alpha_copy > 0 and hasattr(model, 'copy_head') and (compute_copy_labels is not None):
            ...
            for b in range(bsz):
                ...
                copy_labels = compute_copy_labels(src_ids, tgt_ids, output_mask=tgt_mask.unsqueeze(0), ignore_token_id=pad_id)
        ...
        if self.beta_struct > 0 and hasattr(model, 'contrastive_loss_fn'):
            struct_loss = model.contrastive_loss_fn(hidden_states, hidden_states, input_mask.to(dtype=torch.long), output_mask.to(dtype=torch.long))
        total_loss = lm_loss + self.alpha_copy * copy_loss + self.beta_struct * struct_loss
        return (total_loss, outputs) if return_outputs else total_loss

class VulnerabilityDataset(Dataset):

    def __init__(self, data_path: str, tokenizer, max_length: int=2048, prompt_template: str=None, prompt_format: str='plain'):
    ...
    def __getitem__(self, idx):
        ...
        if self.prompt_format in ('chat', 'auto') and hasattr(self.tokenizer, 'apply_chat_template'):
            ...
            messages_full = [{'role': 'system', 'content': instruction}, {'role': 'user', 'content': user_content}, {'role': 'assistant', 'content': output_text}]
            prompt = self.tokenizer.apply_chat_template(messages_full, tokenize=False, add_generation_prompt=False)
            messages_prefix = [{'role': 'system', 'content': instruction}, {'role': 'user', 'content': user_content}]
            prefix = self.tokenizer.apply_chat_template(messages_prefix, tokenize=False, add_generation_prompt=True)
        else:
            prompt = self.prompt_template.format(instruction=instruction, input=input_text, output=output_text)
        ...
        try:
            ...
            full_len = None
            prefix_len = None
        tokenized = self.tokenizer(prompt, max_length=self.max_length, truncation=True, padding='max_length', return_tensors='pt', add_special_tokens=False)
        ...
        try:
            ...
            pass
        return {'input_ids': tokenized['input_ids'].squeeze(), 'attention_mask': tokenized['attention_mask'].squeeze(), 'labels': labels.squeeze()}

class LoRATrainer:

    def __init__(self, model_name: str='deepseek-ai/deepseek-coder-6.7b-base', lora_config: Dict=None, training_args: Dict=None, quantization: str='8bit', device: str='cuda'):
        self.device = device
        self.model_name = model_name
        self.quantization = quantization
        if lora_config is None:
            lora_config = {'r': 8, 'lora_alpha': 32, 'target_modules': ['q_proj', 'v_proj', 'k_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj'], 'lora_dropout': 0.05, 'bias': 'none', 'task_type': TaskType.CAUSAL_LM}
        self.lora_config = lora_config
        if training_args is None:
            training_args = {'output_dir': './checkpoints', 'num_train_epochs': 3, 'per_device_train_batch_size': 4, 'per_device_eval_batch_size': 4, 'gradient_accumulation_steps': 4, 'learning_rate': 0.0002, 'warmup_steps': 100, 'logging_steps': 10, 'save_steps': 500, 'eval_steps': 500, 'save_total_limit': 3, 'fp16': True, 'eval_strategy': 'steps', 'load_best_model_at_end': True, 'report_to': 'wandb'}
    ...
    def _load_model_and_tokenizer(self):
        ...
        try:
            ...
            tokenizer_kwargs = {'trust_remote_code': True, 'padding_side': 'right', 'use_fast': True}
            try:
            ...
            if is_local:
                ...
                try:
                    self.tokenizer = AutoTokenizer.from_pretrained(self.model_name, trust_remote_code=True, padding_side='right', use_fast=False)
        ...
        try:
            ...
            if quant_mode == '8bit':
                try:
                    ...
                    raise
            self.model = AutoModelForCausalLM.from_pretrained(self.model_name, trust_remote_code=True, **extra_model_kwargs, device_map={'': local_rank}, torch_dtype=torch.float16, low_cpu_mem_usage=True)
        ...
        print(f'  Total parameters: {param_count:,} ({param_count / 1000000000.0:.2f}B)')
    ...
    def train(self, train_data_path: str, val_data_path: Optional[str]=None, output_dir: str='./lora_model', wandb_project: Optional[str]=None, prompt_format: str='plain', max_length: int=2048, alpha_copy: float=0.0, beta_struct: float=0.0, struct_temperature: float=0.07):
        ...
        train_dataset = VulnerabilityDataset(train_data_path, self.tokenizer, max_length=int(max_length), prompt_format=prompt_format)
        val_dataset = None
        if val_data_path:
            val_dataset = VulnerabilityDataset(val_data_path, self.tokenizer, max_length=int(max_length), prompt_format=prompt_format)
        data_collator = DataCollatorForSeq2Seq(self.tokenizer, model=self.model, padding=True)
        training_args = TrainingArguments(output_dir=output_dir, **self.training_args)
        ...
        if use_aux:
            if CopyMechanismHead is None or ContrastiveStructuralLoss is None:
                raise RuntimeError('Aux losses requested but src.training.auxiliary_losses is not available.')
            ...
            self.model.contrastive_loss_fn = ContrastiveStructuralLoss(hidden_size, temperature=float(struct_temperature)).to(model_device)
        trainer_cls = AuxLossTrainer if use_aux else Trainer
        trainer_kwargs = dict(model=self.model, args=training_args, train_dataset=train_dataset, eval_dataset=val_dataset, data_collator=data_collator)
        if use_aux:
        ...
        if trainer.is_world_process_zero():
            try:
                run_cfg = {'model_name': self.model_name, 'quantization': self.quantization, 'train_data_path': train_data_path, 'val_data_path': val_data_path, 'output_dir': output_dir, 'prompt_format': prompt_format, 'max_length': int(max_length), 'lora_config': _jsonable(self.lora_config), 'training_args_dict': _jsonable(self.training_args), 'hf_training_args': json.loads(training_args.to_json_string()), 'aux_losses': {'alpha_copy': float(alpha_copy), 'beta_struct': float(beta_struct), 'struct_temperature': float(struct_temperature)}}
    ...
    def inference(self, instruction: str, input_text: str, max_new_tokens: int=512, temperature: float=0.7, top_p: float=0.9) -> str:
        prompt = f'Below is an instruction that describes a task, paired with an input that provides further context. Write a response that appropriately completes the request.\n\n### Instruction:\n{instruction}\n\n### Input:\n{input_text}\n\n### Response:\n'
        inputs = self.tokenizer(prompt, return_tensors='pt', truncation=True, max_length=2048).to(self.device)
        self.model.eval()
        with torch.no_grad():
            outputs = self.model.generate(**inputs, max_new_tokens=max_new_tokens, temperature=temperature, top_p=top_p, do_sample=True, pad_token_id=self.tokenizer.pad_token_id, eos_token_id=self.tokenizer.eos_token_id)
        response = self.tokenizer.decode(outputs[0][inputs['input_ids'].shape[1]:], skip_special_tokens=True)
        return response.strip()

def main():
    parser = argparse.ArgumentParser(description='LoRA微调脚本 - 智能合约漏洞检测')
    parser.add_argument('--train_data', type=str, required=True, help='训练数据路径（JSONL格式）')
    parser.add_argument('--val_data', type=str, default=None, help='验证数据路径（JSONL格式）')
    parser.add_argument('--max_length', type=int, default=2048, help='最大序列长度（越小越省显存；建议 OOM 时降到 1024/1536）')
    parser.add_argument('--model_name', type=str, default='deepseek-ai/deepseek-coder-6.7b-base', help='基座模型名称或本地路径 (支持HuggingFace模型名或本地目录路径)')
    parser.add_argument('--model_path', type=str, default=None, help='本地模型路径（如果指定，将覆盖model_name）')
    parser.add_argument('--output_dir', type=str, default='./lora_model', help='输出目录')
    parser.add_argument('--run_name', type=str, default=None, help='可选：手动指定本次运行的子目录名（会被拼到 output_dir 下面）')
    parser.add_argument('--auto_output_subdir', action='store_true', help='可选：自动用主要超参生成子目录名，并输出到你提供的 output_dir/<run_name>/ 下')
    ...
    parser.add_argument('--quantization', type=str, default='8bit', choices=['8bit', 'none'], help="模型加载量化方式：'8bit'（默认，省显存但依赖 bitsandbytes + CUDA 运行时库）或 'none'（不量化）")
    parser.add_argument('--prompt_format', type=str, default='plain', choices=['plain', 'chat', 'auto'], help="Prompt format. For Qwen2.5-Instruct prefer 'chat' (uses tokenizer.apply_chat_template).")
    ...
    print('=' * 50)
    lora_config = {'r': args.lora_r, 'lora_alpha': args.lora_alpha, 'target_modules': ['q_proj', 'v_proj', 'k_proj', 'o_proj', 'gate_proj', 'up_proj', 'down_proj'], 'lora_dropout': args.lora_dropout, 'bias': 'none', 'task_type': TaskType.CAUSAL_LM}
    training_args_dict = {'num_train_epochs': args.epochs, 'per_device_train_batch_size': args.batch_size, 'per_device_eval_batch_size': args.batch_size, 'gradient_accumulation_steps': args.gradient_accumulation, 'learning_rate': args.learning_rate, 'warmup_steps': args.warmup_steps, 'logging_steps': 10, 'save_steps': 500, 'eval_steps': 500, 'save_total_limit': 3, 'fp16': args.fp16, 'eval_strategy': 'steps' if args.val_data else 'no', 'load_best_model_at_end': True if args.val_data else False, 'report_to': 'none' if os.environ.get('WANDB_DISABLED') else 'wandb' if args.wandb_project else 'none', 'disable_tqdm': True if is_distributed and rank != 0 else False, 'gradient_checkpointing': True, 'gradient_checkpointing_kwargs': {'use_reentrant': False}, 'ddp_find_unused_parameters': True if torch.cuda.device_count() > 1 and (args.alpha_copy > 0 or args.beta_struct > 0) else False if torch.cuda.device_count() > 1 else None}
    trainer = LoRATrainer(model_name=args.model_name, lora_config=lora_config, training_args=training_args_dict, quantization=args.quantization, device=args.device)
    trainer.train(train_data_path=args.train_data, val_data_path=args.val_data, output_dir=args.output_dir, wandb_project=args.wandb_project, prompt_format=args.prompt_format, max_length=args.max_length, alpha_copy=args.alpha_copy, beta_struct=args.beta_struct, struct_temperature=args.struct_temperature)
    print('\n' + '=' * 50)
    print('测试推理:')
