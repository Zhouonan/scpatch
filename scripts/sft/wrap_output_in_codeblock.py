...
def normalize_newlines(s: str) -> str:
    return s.replace('\r\n', '\n').replace('\r', '\n')
HEADING_RE = re.compile('^(#{2,4}\\s*Fixed Code\\s*\\n)+', flags=re.IGNORECASE)
...
def main():
    ...
    ap.add_argument('--in_path', type=str, default='/home/user/zn/smart_contract_vulnerability_detection/data/processed/fix_sft/1227/sft.func_only_over2048.jsonl', help='Input JSONL path.')
    ap.add_argument('--out_path', type=str, default='/home/user/zn/smart_contract_vulnerability_detection/data/processed/fix_sft/1227/sft.func_only_over2048.codeblock.jsonl', help='Output JSONL path.')
    ap.add_argument('--model_path', type=str, default='/home/user/zn/models_cache/Qwen2.5-7B-Instruct', help='Tokenizer path for token stats.')
    ap.add_argument('--threshold', type=int, default=2048, help='Report over-threshold counts for full prompt tokens.')
    ap.add_argument('--language_tag', type=str, default='solidity', help='Fence language tag, e.g. solidity.')
