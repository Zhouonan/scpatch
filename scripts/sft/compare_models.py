...
class ModelComparator:
    ...
    def select_random_functions(self, num_samples: int=10, min_lines: int=5, max_lines: int=100, balance_ratio: float=0.5) -> List[str]:
        import random
        ...
        try:
            ...
            selected_ids = []
            if num_positive > 0:
                vulnerable_query = session.query(SmartContractFunction).filter(SmartContractFunction.is_vulnerable == True, SmartContractFunction.slither_result.isnot(None))
                vulnerable_funcs = []
            ...
            if num_negative > 0:
                safe_query = session.query(SmartContractFunction).filter(SmartContractFunction.is_vulnerable == False, SmartContractFunction.slither_result.isnot(None))
                safe_funcs = []
    ...
    def compare_models(self, function_id: str, model_configs: List[Dict], verbose: bool=False) -> Dict:
        print('=' * 80)
        print(f'多模型对比测试')
        print('=' * 80)
        ...
        for (i, config_dict) in enumerate(model_configs, 1):
            ...
            print(f"{'=' * 80}")
            config = AnnotationConfig(api_key=config_dict['api_key'], base_url=config_dict.get('base_url', 'https://api.openai.com/v1'), model=model_name, temperature=config_dict.get('temperature', 0.1), max_tokens=config_dict.get('max_tokens', 2000), verbose=verbose)
            import time
            ...
            if annotation:
                result = {'model': model_name, 'config': config_dict, 'annotation': annotation, 'stats': annotator.stats.copy(), 'elapsed_time': elapsed_time}
                results.append(result)
                print(f'✅ {model_name} 标注完成 (耗时: {elapsed_time:.2f}秒)')
            else:
                print(f'❌ {model_name} 标注失败 (耗时: {elapsed_time:.2f}秒)')
                results.append({'model': model_name, 'config': config_dict, 'annotation': None, 'error': 'Annotation failed', 'elapsed_time': elapsed_time})
        print(f'\n[3/3] 生成对比报告...')
        comparison = self._generate_comparison(function_info={'function_id': function_id, 'function_name': func_name, 'contract_name': contract_name, 'slither_result': slither_says, 'slither_details': slither_result.get('vulnerability_details', [])}, results=results)
    ...
    def _get_function_data(self, function_id: str) -> Dict:
        ...
        try:
            from src.database.models import SmartContractFunction
            func = session.query(SmartContractFunction).filter_by(sample_id=function_id).first()
            if not func:
                return None
            if func.raw_data:
                func_data = func.raw_data
            else:
                func_data = {'function_name': func.function_name, 'function_code': func.function_code, 'contract_context': func.contract_context or {}, 'called_functions': func.called_functions or [], 'slither_result': func.slither_result or {}}
            return func_data
        finally:
            session.close()

    def _generate_comparison(self, function_info: Dict, results: List[Dict]) -> Dict:
        comparison = {'function': function_info, 'models': [], 'summary': {}}
        for result in results:
            if result.get('annotation'):
                ann = result['annotation']
                comparison['models'].append({'model': result['model'], 'label': ann.get('label'), 'severity': ann.get('severity', 0), 'confidence': ann.get('confidence', 0), 'vulnerability_types': ann.get('vulnerability_types', []), 'agrees_with_slither': ann.get('slither_agreement', None), 'analysis': ann.get('analysis', ''), 'reasoning': ann.get('reasoning', ''), 'slither_critique': ann.get('slither_critique', ''), 'tokens_used': result['stats'].get('total_tokens_used', 0), 'elapsed_time': result.get('elapsed_time', 0)})
            else:
                comparison['models'].append({'model': result['model'], 'error': result.get('error', 'Unknown error'), 'elapsed_time': result.get('elapsed_time', 0)})
        successful_models = [m for m in comparison['models'] if 'error' not in m]
        if successful_models:
            labels = [m['label'] for m in successful_models]
            comparison['summary'] = {'total_models': len(results), 'successful': len(successful_models), 'failed': len(results) - len(successful_models), 'agree_on_vulnerable': labels.count('vulnerable'), 'agree_on_safe': labels.count('safe'), 'consensus': all((l == labels[0] for l in labels)) if labels else False, 'avg_severity': sum((m['severity'] for m in successful_models)) / len(successful_models), 'avg_confidence': sum((m['confidence'] for m in successful_models)) / len(successful_models), 'avg_tokens': sum((m['tokens_used'] for m in successful_models)) / len(successful_models), 'avg_elapsed_time': sum((m['elapsed_time'] for m in successful_models)) / len(successful_models)}
        return comparison
    ...
    def print_comparison(self, comparison: Dict):
        ...
        for model in comparison['models']:
            if 'error' in model:
                table_data.append([model['model'], '❌ 失败', '-', '-', '-', '-', f"{model.get('elapsed_time', 0):.2f}s", model['error'][:20]])
            else:
                agree_symbol = '✅' if model['agrees_with_slither'] else '⚠️'
                table_data.append([model['model'], model['label'], f"{model['severity']:.1f}", f"{model['confidence']:.2f}", f'{agree_symbol}', model['tokens_used'], f"{model['elapsed_time']:.2f}s", '✓'])
    ...
    def batch_compare(self, function_ids: List[str], model_configs: List[Dict], verbose: bool=False, output_dir: str=None) -> List[Dict]:
        print('=' * 80)
        ...
        for (i, func_id) in enumerate(function_ids, 1):
            ...
            print(f"{'#' * 80}\n")
            comparison = self.compare_models(function_id=func_id, model_configs=model_configs, verbose=verbose)
            if comparison:
    ...
    def _print_batch_summary(self, all_comparisons: List[Dict]):
        ...
        for comparison in all_comparisons:
            for model_result in comparison['models']:
                ...
                if model_name not in model_stats:
                    model_stats[model_name] = {'total': 0, 'correct_vs_slither': 0, 'disagreements': 0, 'avg_confidence': 0, 'avg_severity': 0, 'total_tokens': 0, 'vulnerable_calls': 0, 'safe_calls': 0}
        ...
        for (model_name, stats) in sorted(model_stats.items()):
            table_data.append([model_name, stats['total'], f"{stats['agreement_rate'] * 100:.1f}%", f"{stats['avg_confidence']:.2f}", f"{stats['avg_severity']:.1f}", f"{stats['vulnerable_calls']}/{stats['safe_calls']}", f"{stats['avg_elapsed_time']:.2f}s", f"{stats['total_tokens']:.0f}"])
...
def main():
    ...
    parser.add_argument('--function-id', type=str, help='函数的sample_id（与--auto-select互斥）')
    parser.add_argument('--auto-select', type=int, metavar='NUM', help='自动随机选择NUM个函数进行测试')
    parser.add_argument('--min-lines', type=int, default=5, help='函数最小行数（用于auto-select，默认5）')
    parser.add_argument('--max-lines', type=int, default=100, help='函数最大行数（用于auto-select，默认100）')
    parser.add_argument('--balance', type=float, default=0.5, help='正样本（vulnerable）比例（用于auto-select，默认0.5表示1:1）')
    parser.add_argument('--db-path', type=str, default='sqlite:///smart_contracts.db', help='数据库路径')
    parser.add_argument('--models-config', type=str, help='模型配置JSON文件路径')
    parser.add_argument('--models', type=str, nargs='+', help='要测试的模型名称列表')
    parser.add_argument('--api-key', type=str, help='API Key')
    parser.add_argument('--base-url', type=str, default='https://api.openai.com/v1', help='API Base URL')
    parser.add_argument('--output', type=str, help='保存对比结果的JSON文件路径')
    parser.add_argument('--verbose', action='store_true', help='显示LLM的完整响应')
    ...
    if args.models_config:
        ...
        for model in args.models:
            model_configs.append({'model': model, 'api_key': api_key, 'base_url': args.base_url, 'temperature': 0.1, 'max_tokens': 2000})
    else:
    ...
    try:
        if args.auto_select:
            function_ids = comparator.select_random_functions(num_samples=args.auto_select, min_lines=args.min_lines, max_lines=args.max_lines, balance_ratio=args.balance)
            if not function_ids:
                print('错误: 没有找到符合条件的函数')
                sys.exit(1)
            all_comparisons = comparator.batch_compare(function_ids=function_ids, model_configs=model_configs, verbose=args.verbose, output_dir=args.output if args.output else None)
            if args.output:
                import os
                summary_file = os.path.join(args.output, 'summary.json')
                with open(summary_file, 'w', encoding='utf-8') as f:
                    json.dump({'total_functions': len(all_comparisons), 'comparisons': all_comparisons}, f, indent=2, ensure_ascii=False)
                print(f'\n💾 综合报告已保存到: {summary_file}')
        else:
            comparison = comparator.compare_models(function_id=args.function_id, model_configs=model_configs, verbose=args.verbose)
            if comparison:
