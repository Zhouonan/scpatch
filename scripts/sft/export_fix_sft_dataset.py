...
def _bucket_keys_from_display_types(
    display_types: List[str],
    mode: str,
    primary_strategy: str = "order",
    bucket_sizes: Optional[Dict[str, int]] = None,
) -> List[str]:
\
\
\
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--only-successful", action="store_true", default=False,
                        help="仅导出编译与Slither都通过的修复")
    parser.add_argument("--min-quality", type=float, default=None,
                        help="按修复质量分数下限过滤（需要存在 fix_quality_score）")
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="划分比例列表（支持任意份数），例如: --split-ratios 0.8 0.1 0.05 0.05。若提供则忽略 train/val/test-ratio。",
    )
    parser.add_argument(
        "--split-names",
        type=str,
        nargs="*",
        default=None,
        help="与 split-ratios 对应的 split 名称列表，例如: --split-names train val test extra。默认自动生成 train/val/test/split_3...",
    )
    ...
    parser.add_argument("--include-explanation", action="store_true", default=False,
                        help="输出中附带修复说明")
    parser.add_argument("--save-raw", action="store_true", default=False,
                        help="同时导出原始字段 JSON 方便调试")
    parser.add_argument("--stratify-by-type", action="store_true", default=False,
                        help="按漏洞类型分层划分 train/val/test，避免类型分配不均（仍输出一套 train/val/test.jsonl）")
    parser.add_argument("--primary-bucket-strategy", type=str, default="order", choices=["order", "rare"],
                        help="当需要 primary 分桶时的主桶选择策略：order=按固定优先级；rare=若样本含多个桶则优先选择全局更稀有的小桶")
...
\
    ...
    if args.split_names is not None and len(args.split_names) > 0:
        ...
        if len(names) != len(ratios):
            raise ValueError(
                f"--split-names 数量({len(names)})必须与 --split-ratios 数量({len(ratios)})一致"
            )
    else:
...
def fetch_fix_pairs(db: DBManager,
...
\
    ...
    try:
        ...
        query = session.query(VulnerabilityFix, SmartContractFunction).join(
            SmartContractFunction, VulnerabilityFix.function_id == SmartContractFunction.id
        )

        if only_successful:
            query = query.filter(VulnerabilityFix.compiles.is_(True))\
                         .filter(VulnerabilityFix.slither_passed.is_(True))
        ...
        for fix, func_obj in results:
            ...
            pairs.append({
                ...
                "dataset_name": func_obj.dataset_name,
                "code_slice": code_slice,
                "fixed_code": fix.fixed_code,
                "vulnerability_types": fix.vulnerabilities_fixed or [],
                "swc_ids": getattr(fix, "swc_ids", None) or [],
                "is_training": None,             
                "is_validation": None,
                "is_test": None,
                "quality_checked": fix.is_verified,
                "quality_score": fix.fix_quality_score,
                "created_at": str(fix.created_at),
                "annotation": func_obj.llm_audit,
                ...
                "fix_quality_score": fix.fix_quality_score,
            })
        return pairs
    finally:
        session.close()
...
def _stratified_split_dataset(
    ...
    split_ratios: List[float],
    seed: int,
) -> Tuple[List[List[Dict[str, Any]]], Dict[str, Dict[str, int]]]:
\
\
\
...
def main():
    ...
    if args.stratify_by_type:
        ...
        strat_mode = "primary"
        def key_fn(p):
            return _bucket_keys_from_display_types(
                p.get("_export_vulnerability_types") or [],
                mode=strat_mode,
                primary_strategy=args.primary_bucket_strategy,
                bucket_sizes=bucket_sizes,
            )[0]

        splits, stats = _stratified_split_dataset(
            pairs,
            key_fn=key_fn,
        ...
        with open(output_dir / "stratified_split_summary.json", "w", encoding="utf-8") as f:
            json.dump(
                {
                    "mode": "stratify_by_type",
                    "bucket_order": BUCKET_ORDER + ["OTHER"],
                    "split_names": split_names,
                    "split_ratios": split_ratios,
                    "stats_by_bucket": stats,
                    "primary_bucket_strategy": args.primary_bucket_strategy,
                    "primary_bucket_presence_sizes": bucket_sizes,
                },
                f,
                indent=2,
                ensure_ascii=False,
