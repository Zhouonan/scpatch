...
DEFAULT_ALLOWED_TYPES: List[str] = ['REENTRANCY', 'ACCESS_CONTROL', 'ARITHMETIC', 'UNCHECKED_CALL', 'BAD_RANDOMNESS', 'FRONT_RUNNING', 'DOS', 'TX_ORIGIN', 'TIMESTAMP_DEPENDENCE', 'DELEGATECALL', 'UNINITIALIZED_STORAGE', 'STORAGE_CORRUPTION']
DEFAULT_TYPE_SYNONYMS: Dict[str, str] = {'reentrancy': 'REENTRANCY', 'reentrancy-eth': 'REENTRANCY', 're-entrancy': 'REENTRANCY', 'access control': 'ACCESS_CONTROL', 'authorization': 'ACCESS_CONTROL', 'auth': 'ACCESS_CONTROL', 'integer overflow/underflow': 'ARITHMETIC', 'integer overflow': 'ARITHMETIC', 'integer underflow': 'ARITHMETIC', 'overflow': 'ARITHMETIC', 'underflow': 'ARITHMETIC', 'arithmetic': 'ARITHMETIC', 'arithmetic error': 'ARITHMETIC', 'unchecked low-level calls': 'UNCHECKED_CALL', 'unchecked low level calls': 'UNCHECKED_CALL', 'unchecked call': 'UNCHECKED_CALL', 'unchecked return value': 'UNCHECKED_CALL', 'bad randomness': 'BAD_RANDOMNESS', 'predictable game outcome': 'BAD_RANDOMNESS', 'weak randomness': 'BAD_RANDOMNESS', 'front running': 'FRONT_RUNNING', 'front-running': 'FRONT_RUNNING', 'transaction ordering dependence': 'FRONT_RUNNING', 'tod': 'FRONT_RUNNING', 'denial of service': 'DOS', 'dos': 'DOS', 'tx.origin': 'TX_ORIGIN', 'tx origin': 'TX_ORIGIN', 'tx_origin': 'TX_ORIGIN', 'timestamp dependence': 'TIMESTAMP_DEPENDENCE', 'block.timestamp dependence': 'TIMESTAMP_DEPENDENCE', 'time manipulation': 'TIMESTAMP_DEPENDENCE', 'miner manipulation': 'TIMESTAMP_DEPENDENCE', 'delegatecall': 'DELEGATECALL', 'uninitialized storage pointer': 'UNINITIALIZED_STORAGE', 'uninitialized storage': 'UNINITIALIZED_STORAGE', 'uninitialized state': 'UNINITIALIZED_STORAGE', 'storage corruption': 'STORAGE_CORRUPTION'}
VULN_TYPE_RE = re.compile('(?mi)^\\s*-\\s*Type:\\s*(.+?)\\s*$')
...
def _normalize_vuln_type_one(t: str, synonyms: Dict[str, str], allowed: List[str]) -> Optional[str]:
    ...
    if 'overflow' in key or 'underflow' in key or 'precision' in key or ('division' in key) or ('rounding' in key) or ('truncation' in key) or ('arithmetic' in key) or ('math' in key) or ('calculation' in key):
        return 'ARITHMETIC' if 'ARITHMETIC' in allowed else None
    if 'unchecked' in key or 'low level call' in key or 'low-level call' in key or ('external call' in key) or ('call return' in key) or ('send' in key) or ('transfer' in key):
    ...
    if 'denial of service' in key or key == 'dos' or 'gas exhaustion' in key or ('gas grief' in key) or ('gas consumption' in key) or ('unbounded loop' in key) or ('infinite loop' in key) or ('gas limit' in key):
    ...
    canon_guess = re.sub('[^a-z0-9]+', '_', key).strip('_').upper()
...
def analyze_jsonl(path: Path, allowed: List[str], synonyms: Dict[str, str]) -> AnalyzeResult:
    ...
    return AnalyzeResult(total=total, missing_type=missing_type, unmappable=unmappable, raw_type_counts=dict(raw_counter.most_common()), norm_type_counts=dict(norm_counter.most_common()), raw_to_norm_counts={k: dict(v.most_common()) for (k, v) in raw_to_norm.items()}, unmappable_raw_type_counts=dict(unmappable_raw.most_common()), multi_label_samples=multi_label_samples)
...
def write_id_map(path_in: Path, path_out: Path, allowed: List[str], synonyms: Dict[str, str]) -> None:
    ...
    with open(path_in, 'r', encoding='utf-8') as fin, open(path_out, 'w', encoding='utf-8') as fout:
        for line in fin:
            ...
            fout.write(json.dumps({'id': sid, 'raw_types': raw_types, 'norm_types': norm_types}, ensure_ascii=False) + '\n')
...
def main():
    ...
    report = {'input': str(in_path), 'allowed_types': allowed, 'summary': {'total_samples': res.total, 'missing_type_samples': res.missing_type, 'unmappable_samples': res.unmappable, 'multi_label_samples': res.multi_label_samples, 'raw_type_vocab_size': len(res.raw_type_counts), 'norm_type_vocab_size': len(res.norm_type_counts)}, 'raw_type_counts': res.raw_type_counts, 'norm_type_counts': res.norm_type_counts, 'unmappable_raw_type_counts': res.unmappable_raw_type_counts, 'raw_to_norm_counts': res.raw_to_norm_counts}
    if args.report_out:
