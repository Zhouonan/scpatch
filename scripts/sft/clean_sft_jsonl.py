...
DEFAULT_SHORT_INSTRUCTION = (
    "You are an expert Solidity smart contract security auditor. "
    "Fix vulnerabilities while preserving functionality. "
    "Output the complete fixed function code."
)
...
def drop_output_heading(s: str) -> str:
    ...
    s2 = re.sub(r"^(#{2,4}\s*Fixed Code\s*\n)+", "", s2, flags=re.IGNORECASE)
    return s2.lstrip("\n")
...
def main():
    ...
    ap.add_argument(
        "--out_path",
        type=str,
        default="/home/user/zn/smart_contract_vulnerability_detection/data/processed/fix_sft/1227/sft.cleaned.jsonl",
    )
    ap.add_argument(
        "--model_path",
        type=str,
    ...
    with out_path.open("w", encoding="utf-8") as wf:
        for item in iter_jsonl(in_path):
            ...
            input2 = clean_text(
                input_text,
                tab_spaces=int(args.tab_spaces),
                max_blank_lines=int(args.max_blank_lines),
                drop_fixed_code_heading=False,                   
            )
            output2 = clean_text(
                output_text,
                tab_spaces=int(args.tab_spaces),
                max_blank_lines=int(args.max_blank_lines),
                drop_fixed_code_heading=bool(args.drop_fixed_code_heading),
            )
