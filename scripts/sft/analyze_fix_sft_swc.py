...
def _norm_key(s: str) -> str:
    ...
    s = re.sub('\\s+', ' ', s).strip()
    return s
SWC_TITLES: Dict[str, str] = {'SWC-101': 'Integer Overflow and Underflow', 'SWC-104': 'Unchecked Call Return Value', 'SWC-106': 'Unprotected SELFDESTRUCT Instruction', 'SWC-107': 'Reentrancy', 'SWC-109': 'Uninitialized Storage Pointer', 'SWC-110': 'Assert Violation', 'SWC-111': 'Use of Deprecated Solidity Functions', 'SWC-112': 'Delegatecall to Untrusted Callee', 'SWC-113': 'DoS with Failed Call', 'SWC-114': 'Transaction Order Dependence', 'SWC-115': 'Authorization through tx.origin', 'SWC-116': 'Block values as a proxy for time', 'SWC-117': 'Signature Malleability', 'SWC-120': 'Weak Sources of Randomness from Chain Attributes', 'SWC-121': 'Missing Protection against Signature Replay Attacks', 'SWC-122': 'Lack of Proper Signature Verification', 'SWC-124': 'Write to Arbitrary Storage Location', 'SWC-126': 'Insufficient Gas Griefing', 'SWC-128': 'DoS With Block Gas Limit', 'SWC-132': 'Unexpected Ether Balance', 'SWC-133': 'Hash Collisions With Multiple Variable Length Arguments'}
...
def map_raw_type_to_swcs(raw_type: str) -> Set[str]:
    ...
    if 'signature verification' in key:
        out.add('SWC-122')
    if 'arbitrary storage write' in key or 'write to arbitrary storage' in key or 'arbitrary write' in key or ('storage corruption' in key) or ('storage collision' in key) or ('out of bounds write' in key) or ('state variable overwrite' in key) or ('state variable corruption' in key):
        out.add('SWC-124')
...
def analyze(input_path: Path, exclude_swcs: Set[str], max_examples_per_swc: int=30) -> SwcReport:
    ...
    return SwcReport(total_samples=total_samples, missing_type_samples=missing_type_samples, excluded_swcs=sorted(exclude_swcs), swc_sample_counts=swc_sample_counts, swc_label_counts=swc_label_counts, swc_to_raw_examples=swc_to_raw_examples, unmapped_raw_type_counts=unmapped_raw_type_counts)
...
def write_id_map(input_path: Path, out_path: Path, exclude_swcs: Set[str]) -> None:
    ...
    with open(input_path, 'r', encoding='utf-8') as fin, open(out_path, 'w', encoding='utf-8') as fout:
        for line in fin:
            ...
            fout.write(json.dumps({'id': sid, 'raw_types': raw_types, 'swcs': sorted(swcs)}, ensure_ascii=False) + '\n')
...
def main():
    ...
    report_obj = {'input': str(input_path), 'excluded_swcs': rep.excluded_swcs, 'swc_titles': {k: SWC_TITLES.get(k, '') for k in rep.swc_sample_counts.keys()}, 'summary': {'total_samples': rep.total_samples, 'missing_type_samples': rep.missing_type_samples, 'mapped_swc_vocab_size': len(rep.swc_sample_counts), 'unmapped_raw_type_vocab_size': len(rep.unmapped_raw_type_counts)}, 'swc_sample_counts': rep.swc_sample_counts, 'swc_label_counts': rep.swc_label_counts, 'swc_to_raw_examples': rep.swc_to_raw_examples, 'unmapped_raw_type_counts': rep.unmapped_raw_type_counts}
    if args.report_out:
