...
def normalize_newlines(s: str) -> str:
    return s.replace('\r\n', '\n').replace('\r', '\n')
FUNC_RE = re.compile('\\*\\*Function:\\*\\*\\s*([A-Za-z_][A-Za-z0-9_]*)')
...
def find_function_span(code: str, func_name: str) -> Optional[Tuple[int, int]]:
    pat = re.compile(f'\\bfunction\\s+{re.escape(func_name)}\\s*\\(', flags=re.MULTILINE)
    m = pat.search(code)
    if not m:
        return None
    start = m.start()
...
def main():
    ...
    ap.add_argument('--out_path', type=str, default='/home/user/zn/smart_contract_vulnerability_detection/data/processed/fix_sft/1227/sft.func_only_over2048.jsonl')
