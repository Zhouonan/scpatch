...
class AnnotationPipeline:
    ...
    def __init__(self, db_manager: DBManager, annotator: LLMAnnotator, save_interval: int=10, concurrency: int=5):
        ...
        with tqdm(total=total, desc='标注进度') as pbar:
            for coro in asyncio.as_completed(tasks):
                ...
                if annotation:
                    ...
                    annotated_count += 1
                    pbar.set_postfix({'success': annotated_count, 'failed': failed_count, 'current': f'{contract_name}.{func_name}'})
                else:
                    failed_count += 1
                    pbar.set_postfix({'success': annotated_count, 'failed': failed_count, 'current': f'FAILED: {contract_name}.{func_name}'})
    ...
    def run(self, dataset_types: Optional[List[str]]=None, dataset_names: Optional[List[str]]=None, limit: Optional[int]=None, only_with_slither: bool=False, skip_already_annotated: bool=True, select_vulnerable_functions: bool=False):
        print('=' * 80)
        print('LLM标注流水线 - 阶段1')
        print('=' * 80)
        print('\n[1/4] 查询数据库...')
        functions = self._query_functions(dataset_types=dataset_types, dataset_names=dataset_names, only_with_slither=only_with_slither, skip_already_annotated=skip_already_annotated, select_vulnerable_functions=select_vulnerable_functions)
        if not functions:
            print('没有需要标注的数据')
            return
        if limit:
    ...
    def _query_functions(self, dataset_types: Optional[List[str]], dataset_names: Optional[List[str]], only_with_slither: bool, skip_already_annotated: bool, select_vulnerable_functions: bool=False) -> List:
        ...
        try:
            ...
            if select_vulnerable_functions:
                query = query.filter(or_(SmartContractFunction.is_vulnerable == True, text("json_extract(slither_result, '$.is_vulnerable') = true")))
            return query.all()
        finally:
            session.close()
    ...
    def _prepare_function_data(self, functions: List) -> List[Dict]:
        ...
        for func in functions:
            func_data = {'function_name': func.function_name, 'function_code': func.function_code, 'function_signature': func.function_signature, 'contract_context': func.contract_context or {}, 'called_functions': func.called_functions or [], 'caller_functions': func.caller_functions or [], 'slither_result': func.slither_result or {}, 'start_line': func.start_line, 'end_line': func.end_line, 'contract_path': func.contract_path, 'label': func.label}
...
def main():
    ...
    parser.add_argument('--include-without-slither', action='store_true', help='包含没有Slither结果的数据')
    parser.add_argument('--reannotate', action='store_true', help='重新标注已有LLM标注的数据')
    parser.add_argument('--select-vulnerable', action='store_true', default=False, help='选取漏洞或者slither检测为漏洞的函数')
    ...
    config = AnnotationConfig(api_key=api_key, base_url=args.base_url, model=args.model, temperature=args.temperature, verbose=args.verbose)
    annotator = LLMAnnotator(config)
    pipeline = AnnotationPipeline(db_manager=db_manager, annotator=annotator, save_interval=args.save_interval, concurrency=args.concurrency)
    try:
        pipeline.run(dataset_types=args.dataset_types, dataset_names=args.dataset_names, limit=args.limit, only_with_slither=not args.include_without_slither, skip_already_annotated=not args.reannotate, select_vulnerable_functions=args.select_vulnerable)
