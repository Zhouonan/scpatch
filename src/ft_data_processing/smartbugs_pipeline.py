...
\
    ...
    def _normalize_config(self, config: Dict) -> Dict:
        ...
        if 'smartbugs_wild_dir' in config:
            datasets.append({
                'name': 'smartbugs_wild',
                'path': config['smartbugs_wild_dir'],
                'type': 'wild',
                'sample_limit': config.get('wild_sample_limit', None),
                'quality_filter': True,
            })
        ...
        if 'smartbugs_curated_dir' in config:
            datasets.append({
                ...
                'path': config['smartbugs_curated_dir'],
                'type': 'curated',
                'sample_limit': config.get('curated_sample_limit', None),
    ...
    def run_full_pipeline(self):
        ...
        function_dataset = self.function_processing(
            all_contracts, 
...
\
        ...
        for dataset_config in self.config['datasets']:
            ...
            if dataset_config['type'] == 'wild':
                ...
                processor = SmartBugsProcessor(
                    wild_dir=None,
                    curated_dir=dataset_config['path'],
                    solidifi_dir=None,
                    output_dir=str(output_dir)
                )
                ...
                processor = SmartBugsProcessor(
                    wild_dir=None,
                    curated_dir=None,
                    solidifi_dir=dataset_config['path'],
                    output_dir=str(output_dir)
                )
                ...
                processor = ScrawlDProcessor(
                    scrawld_dir=dataset_config['path'],
                    output_dir=str(output_dir),
...
\
        ...
        processor = FunctionLevelProcessor(
            use_slither=self.config.get('use_slither', False),
        ...
        for batch_idx in range(num_batches):
            ...
            for i, contract in enumerate(batch_contracts):
                ...
                try:
                    ...
                    returned_functions = processor.process_contract_to_functions(
                        contract_file=contract.file_path,
        ...
        processor.save_function_contexts(
            all_functions,
            str(output_dir / 'all_functions.json')
        )
...
\
    ...
    def create_splits(self, function_dataset):
        ...
        curated_funcs = [f for f in function_dataset 
                        if get_dataset_type(f) == 'curated']
        wild_funcs = [f for f in function_dataset 
                     if get_dataset_type(f) == 'wild']
        solidifi_funcs = [f for f in function_dataset 
                         if get_dataset_type(f) == 'solidifi']
        ...
        for funcs, label in [(wild_vulnerable, '有漏洞'), (wild_safe, '安全')]:
            ...
            print(f"    {label}: Train={len(funcs[:train_end])}, "
                  f"Val={len(funcs[train_end:val_end])}, "
                  f"Test={len(funcs[val_end:])}")
        ...
        splits = {
            'train': [f if isinstance(f, dict) else vars(f) for f in train_funcs],
            'val': [f if isinstance(f, dict) else vars(f) for f in val_funcs],
            'test': [f if isinstance(f, dict) else vars(f) for f in test_funcs]
        }
        ...
        for split_name, split_data in splits.items():
            ...
            print(f"    {split_name}: {len(split_data)} 个样本 "
                  f"(漏洞:{vuln_count}, 安全:{safe_count}) -> {split_file}")
        
                
        summary = {
            'train_size': len(train_funcs),
            'val_size': len(val_funcs),
            'test_size': len(test_funcs),
            'total': len(function_dataset),
            'curated_in_test': len(curated_funcs),
            'vulnerable_total': len(vulnerable_funcs),
            'safe_total': len(safe_funcs),
    ...
    def print_summary(self):
        ...
        if summary_file.exists():
            ...
            print()
            print(f"训练集: {summary['train_size']} 个函数 "
                  f"({summary['train_size']/summary['total']*100:.1f}%)")
            print(f"验证集: {summary['val_size']} 个函数 "
                  f"({summary['val_size']/summary['total']*100:.1f}%)")
            print(f"测试集: {summary['test_size']} 个函数 "
                  f"({summary['test_size']/summary['total']*100:.1f}%)")
...
def main():
    ...
    config = {
        ...
        'output_dir': PROCESSED_DIR / 'scrawld_smoke_vulnonly',
        
                        
        'datasets': [
            {
                'name':'scrawld',
                'path':'/home/user/zn/ScrawlD',
                'type':'scrawld',
                'sample_limit':200,
                'quality_filter': False
            }
        ...
        'batch_size': 500,                            
        'save_to_db': True,                           
        'db_path': 'sqlite:///smart_contracts.db',        
