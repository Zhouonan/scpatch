...
class VulnSpec:
    ...
    location_kind: str
VULN_SPECS: Tuple[VulnSpec, ...] = (VulnSpec(key='ARITHMETIC_OVERFLOW_UNDERFLOW', name='Arithmetic (Integer Overflow and Underflow)', category='arithmetic', threshold=2, scrawld_tokens=('ARTHM',), location_kind='line'), VulnSpec(key='DENIAL_OF_SERVICE', name='Denial of Service', category='denial_of_service', threshold=1, scrawld_tokens=('DOS',), location_kind='line'), VulnSpec(key='REENTRANCY', name='Reentrancy', category='reentrancy', threshold=2, scrawld_tokens=('RENT',), location_kind='function'), VulnSpec(key='TIME_MANIPULATION', name='Time Manipulation (Block values as a proxy for time)', category='time_manipulation', threshold=2, scrawld_tokens=('TimeM',), location_kind='line'), VulnSpec(key='TIMESTAMP_ORDERING', name='Timestamp Ordering (Transaction Order Dependence)', category='front_running', threshold=1, scrawld_tokens=('TimeO',), location_kind='line'), VulnSpec(key='TX_ORIGIN_AUTHORIZATION', name='Authorization through tx.origin', category='access_control', threshold=2, scrawld_tokens=('Tx-Origin', 'TX-Origin'), location_kind='line'), VulnSpec(key='UNCHECKED_CALL_RETURN_VALUE', name='Unhandled Exception (Unchecked Call Return Value)', category='unchecked_low_level_calls', threshold=1, scrawld_tokens=('UE',), location_kind='line'))
TOKEN_TO_SPEC: Dict[str, VulnSpec] = {tok: spec for spec in VULN_SPECS for tok in spec.scrawld_tokens}
...
def _parse_scrawld_res_all(res_all_path: Path) -> Dict[str, Dict[str, Dict[ScrawlDLoc, Set[str]]]]:
...
class ScrawlDProcessor:
    ...
    def __init__(self, scrawld_dir: str, output_dir: str='./processed_data', thresholds: Optional[Dict[str, int]]=None, include_unknown_mappings: bool=False):
    ...
    def _apply_thresholds(self, contract_file: str) -> Dict[str, List[Tuple[ScrawlDLoc, Set[str]]]]:
        ...
        for sol_file in tqdm(sol_files, desc='  处理 ScrawlD'):
            try:
                ...
                for (func_sig, start_line, end_line) in ranges:
                    ...
                    func_obj = {'name': name, 'signature': func_sig, 'code': func_code, 'lines': [start_line, end_line], 'vulnerabilities': [], 'is_vulnerable': False}
                ...
                def add_vuln(func: Dict, spec: VulnSpec, lines: List[int], tools: Set[str], raw_type: str):
                    for v in func['vulnerabilities']:
                        if v.get('category') == spec.category and v.get('raw_type') == raw_type:
                            ...
                            return
                    func['vulnerabilities'].append({'category': spec.category, 'name': spec.name, 'raw_type': raw_type, 'lines': sorted(set(lines)), 'tools': sorted(set(tools))})
                ...
                contract_label = {'has_vulnerability': len(contract_vuln_types) > 0, 'vulnerability_types': sorted(contract_vuln_types), 'vulnerability_names': sorted(contract_vuln_names), 'source': 'scrawld_res_all', 'thresholds': {k: int(v) for (k, v) in self.thresholds.items()}}
                contract = ContractInfo(file_path=str(sol_file), contract_name=sol_file.stem, solidity_version=version, functions=functions, label=contract_label, metadata={'dataset': 'scrawld', 'ground_truth': True, 'address': addr})
    ...
    def save_contracts(self, contracts: List[ContractInfo], filename: str) -> None:
        ...
        data = []
        for c in contracts:
            data.append({'file_path': c.file_path, 'contract_name': c.contract_name, 'solidity_version': c.solidity_version, 'num_functions': len(c.functions), 'functions': c.functions, 'label': c.label, 'metadata': c.metadata})
