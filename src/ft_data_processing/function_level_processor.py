...
class FunctionLevelProcessor:
    ...
    def __init__(self, use_slither: bool=True, debug: bool=False, enable_deduplication: bool=True, enable_filtering: bool=True):
        ...
        self._dedup_table = {}
        self._stats = {'total_extracted': 0, 'filtered_out': 0, 'deduplicated': 0, 'final_count': 0}
    ...
    def process_contract_to_functions(self, contract_file: str, contract_info: Optional[Dict]=None) -> List[FunctionContext]:
        if self.use_slither:
            return self._process_with_slither(contract_file, contract_info)
        else:
            return self._process_with_regex(contract_file, contract_info)

    def _process_with_slither(self, contract_file: str, contract_info: Optional[Dict]) -> List[FunctionContext]:
        function_contexts = []
        try:
            with self.slither_manager.analyze_contract(contract_file) as slither:
                if not slither:
                    ...
                    return []
                collector = VulnerabilityCollector(slither=slither, debug=self.debug)
                    ...
                    try:
                        ...
                        for (func_idx, function) in enumerate(contract.functions):
                            try:
                                ...
                                slither_result = {'is_vulnerable': len(func_vulns) > 0, 'vulnerability_types': vuln_types, 'vulnerability_count': len(vuln_types), 'vulnerability_details': func_vulns}
                                func_context = self._build_function_context(function=function, call_graph=call_graph, contract_context=contract_context, contract_info=contract_info, slither_result=slither_result)
                                if not func_context:
                                    continue
                                (should_filter, filter_reason) = self.should_filter_function(function, func_context.function_code)
                                if should_filter:
                                    self._stats['filtered_out'] += 1
                                    if self.debug:
    ...
    def _process_with_regex(self, contract_file: str, contract_info: Optional[Dict]) -> List[FunctionContext]:
        function_contexts = []
        try:
            ...
            for (func_name, func_data) in functions.items():
                self._stats['total_extracted'] += 1
                func_context = self._build_function_context_regex(func_name=func_name, func_data=func_data, call_graph=call_graph, contract_context=contract_context, contract_file=contract_file, contract_info=contract_info)
                if func_context:
                    if self.enable_filtering:
                        ...
                        dummy = _DummyFunc(name=func_name, metadata=func_context.metadata or {}, visibility=(func_context.metadata or {}).get('visibility', 'unknown'), state_mutability=(func_context.metadata or {}).get('state_mutability', 'unknown'))
        ...
        for (func_idx, function) in enumerate(contract.functions):
            ...
            for caller in function.reachable_from_functions:
                if caller.contract == contract:
                    callers.append({'name': caller.name, 'signature': caller.signature_str, 'visibility': str(caller.visibility)})
            for callee in (getattr(function, 'internal_calls', []) or []) + (getattr(function, 'external_calls_as_expressions', []) or []):
                ...
                if not callee_name:
                    continue
                callees.append({'name': callee_name, 'type': 'internal' if callee in (getattr(function, 'internal_calls', []) or []) else 'external'})
            call_graph[function.name] = {'callers': callers, 'callees': callees}
        return call_graph
        ...
        for (func_name, func_data) in functions.items():
            ...
            for other_name in functions.keys():
                if other_name != func_name and other_name in func_code:
                    callees.append({'name': other_name, 'type': 'internal'})
            for (other_name, other_data) in functions.items():
                if other_name != func_name and func_name in other_data['code']:
                    callers.append({'name': other_name, 'signature': other_name, 'visibility': 'unknown'})
            call_graph[func_name] = {'callers': callers, 'callees': callees}
        return call_graph
    ...
    def _extract_contract_context(self, contract) -> Dict:
        ...
        for var in contract.state_variables:
            try:
                ...
                state_variables.append({'name': var.name, 'type': str(var.type), 'visibility': str(var.visibility), 'code': code})
            except Exception:
                state_variables.append({'name': var.name, 'type': str(var.type), 'visibility': str(var.visibility), 'code': f'// Error extraction: {var.name}'})
        modifiers = []
        for mod in contract.modifiers:
            try:
                ...
                modifiers.append({'name': mod.name, 'code': code})
            except Exception:
                pass
        structures = []
        if hasattr(contract, 'structures'):
            for struct in contract.structures:
                try:
                    ...
                    structures.append({'name': struct.name, 'code': code})
                except Exception:
                    pass
        events = []
        if hasattr(contract, 'events'):
            for event in contract.events:
                try:
                    ...
                    events.append({'name': event.name, 'code': code})
                except Exception:
                    pass
        return {'contract_name': contract.name, 'inheritance': [str(base) for base in contract.inheritance], 'state_variables': state_variables, 'modifiers': modifiers, 'structures': structures, 'events': events, 'num_functions': len(contract.functions)}

    def _extract_contract_context_regex(self, code: str) -> Dict:
        contract_name = 'Unknown'
        match = re.search('contract\\s+(\\w+)', code)
        if match:
            contract_name = match.group(1)
        inheritance = []
        match = re.search('contract\\s+\\w+\\s+is\\s+([^{]+)', code)
        if match:
            inheritance = [i.strip() for i in match.group(1).split(',')]
        state_vars = re.findall('(uint|address|bool|string|mapping)\\s+(\\w+)\\s*;', code)
        modifiers = re.findall('modifier\\s+(\\w+)', code)
        return {'contract_name': contract_name, 'inheritance': inheritance, 'state_variables': [{'name': name, 'type': typ} for (typ, name) in state_vars], 'modifiers': modifiers}
    ...
    def _build_function_context(self, function, call_graph: Dict, contract_context: Dict, contract_info: Optional[Dict], slither_result: Dict) -> Optional[FunctionContext]:
        try:
            ...
            try:
                ...
                for callee in raw_callees:
                    ...
                    try:
                        ...
                        called_funcs.append({'name': callee_name, 'contract_name': callee_contract_name, 'type': 'internal', 'code': code})
                    except Exception:
                        continue
            except Exception as e:
                if self.debug:
            ...
            for caller_info in callers:
                caller_funcs.append({'name': caller_info.get('name', 'unknown'), 'signature': caller_info.get('signature', caller_info.get('name', 'unknown')), 'code_snippet': f"// Caller: {caller_info.get('signature', caller_info.get('name', 'unknown'))}"})
            try:
            ...
            try:
                ...
                contract_file = 'unknown'
            metadata = {'contract_file': contract_file, 'function_name': func_name, 'visibility': str(function.visibility) if hasattr(function, 'visibility') else 'unknown', 'state_mutability': self._safe_get_state_mutability(function), 'is_payable': function.payable if hasattr(function, 'payable') else False}
            if contract_info:
                ...
                for func_info in contract_info['functions']:
                    if func_info.get('name') == func_name:
                        ...
                        if func_info.get('is_vulnerable'):
                            func_label = {'is_vulnerable': True, 'vulnerability_types': [v['category'] for v in func_info.get('vulnerabilities', [])], 'vulnerability_details': func_info.get('vulnerabilities', []), 'source': 'ground_truth'}
                        else:
                            func_label = {'is_vulnerable': False, 'vulnerability_types': [], 'vulnerability_details': [], 'source': 'ground_truth'}
                        break
            return FunctionContext(function_code=func_code, function_name=func_name, function_signature=func_signature, start_line=start_line, end_line=end_line, caller_functions=caller_funcs, called_functions=called_funcs, contract_context=func_contract_context, ast_features=ast_features, metadata=metadata, label=func_label, slither_result=slither_result)
        except Exception as e:
            print(f"Error building context for function {getattr(function, 'name', 'unknown')}: {e}")
            return None

    def _build_function_context_regex(self, func_name: str, func_data: Dict, call_graph: Dict, contract_context: Dict, contract_file: str, contract_info: Optional[Dict]) -> Optional[FunctionContext]:
        try:
            callers = call_graph[func_name]['callers'][:3]
            callees = call_graph[func_name]['callees'][:3]
            caller_funcs = []
            for caller_info in callers:
                caller_funcs.append({'name': caller_info['name'], 'signature': caller_info.get('signature', caller_info['name']), 'code_snippet': f"// Caller: {caller_info['name']}"})
            called_funcs = []
            for callee_info in callees:
                called_funcs.append({'name': callee_info['name'], 'type': callee_info.get('type', 'unknown')})
            ast_features = self._extract_ast_features_regex(func_data['code'])
            metadata = {'contract_file': contract_file, 'function_name': func_name, 'visibility': func_data.get('visibility', 'unknown'), 'state_mutability': func_data.get('state_mutability', 'unknown')}
            if contract_info:
                metadata.update(contract_info)
            start_line = 1
            end_line = 1
                ...
                for func_info in contract_info['functions']:
                    if func_info.get('name') == func_name:
                        ...
                        if func_info.get('is_vulnerable'):
                            ...
                            func_label = {'is_vulnerable': False, 'vulnerability_types': [], 'vulnerability_details': [], 'source': 'ground_truth'}
                        break
            return FunctionContext(function_code=func_data['code'], function_name=func_name, function_signature=func_name, start_line=start_line, end_line=end_line, caller_functions=caller_funcs, called_functions=called_funcs, contract_context=contract_context, ast_features=ast_features, metadata=metadata, label=func_label, slither_result={})
        except Exception as e:
    ...
    def _extract_functions_regex(self, code: str) -> Dict[str, Dict]:
        functions = {}
        pattern = 'function\\s+(\\w+)\\s*\\([^)]*\\)\\s*(public|private|internal|external)?\\s*(view|pure|payable)?\\s*(returns\\s*\\([^)]*\\))?\\s*\\{([^}]*(?:\\{[^}]*\\}[^}]*)*)\\}'
        for match in re.finditer(pattern, code, re.DOTALL):
            func_name = match.group(1)
            visibility = match.group(2) or 'public'
            state_mutability = match.group(3) or 'nonpayable'
            func_body = match.group(5)
            functions[func_name] = {'code': match.group(0), 'body': func_body, 'visibility': visibility, 'state_mutability': state_mutability}
        return functions
    ...
    def _extract_ast_features_slither(self, function) -> Dict:
        try:
            ...
            try:
                ...
                state_vars_written = []
                state_vars_read = []
            features = {'has_loops': bool(re.search('\\b(for|while)\\s*\\(', func_code)), 'has_if_statements': len(nodes) > 1, 'complexity': len(nodes), 'has_external_calls': len(external_calls) > 0, 'has_state_changes': len(state_vars_written) > 0, 'reads_state_vars': len(state_vars_read) > 0, 'writes_state_vars': len(state_vars_written) > 0, 'has_require': 'require(' in func_code, 'uses_tx_origin': 'tx.origin' in func_code, 'uses_delegatecall': 'delegatecall' in func_code, 'is_payable': function.payable if hasattr(function, 'payable') else False, 'is_view': function.view if hasattr(function, 'view') else False, 'is_pure': function.pure if hasattr(function, 'pure') else False, 'visibility': str(function.visibility) if hasattr(function, 'visibility') else 'unknown'}
            return features
        except Exception as e:
            return {'has_loops': False, 'has_if_statements': False, 'complexity': 0, 'has_external_calls': False, 'has_state_changes': False, 'reads_state_vars': False, 'writes_state_vars': False, 'has_require': False, 'uses_tx_origin': False, 'uses_delegatecall': False, 'is_payable': False, 'is_view': False, 'is_pure': False, 'visibility': 'unknown'}

    def _extract_ast_features_regex(self, code: str) -> Dict:
        features = {'has_loops': bool(re.search('\\b(for|while)\\s*\\(', code)), 'has_if_statements': bool(re.search('\\bif\\s*\\(', code)), 'complexity': code.count('{'), 'has_external_calls': bool(re.search('\\.(call|delegatecall|staticcall)\\(', code)), 'has_state_changes': bool(re.search('[a-zA-Z_]\\w*\\s*=', code)), 'has_require': 'require(' in code, 'uses_tx_origin': 'tx.origin' in code, 'uses_delegatecall': 'delegatecall' in code, 'uses_transfer': '.transfer(' in code, 'uses_send': '.send(' in code, 'is_payable': 'payable' in code, 'is_view': 'view' in code, 'is_pure': 'pure' in code}
        return features

    def batch_process_contracts(self, contract_files: List[str], output_file: str, contract_infos: Optional[Dict[str, Dict]]=None):
        ...
        for contract_file in tqdm(contract_files, desc='Processing contracts'):
            ...
            function_contexts = self.process_contract_to_functions(contract_file, contract_info)
    ...
    def save_function_contexts(self, function_contexts: List[FunctionContext], output_file: str):
...
if __name__ == '__main__':
    ...
    function_contexts = processor.process_contract_to_functions(contract_file='path/to/contract.sol')
    print(f'提取了 {len(function_contexts)} 个函数')
    contract_files = ['contract1.sol', 'contract2.sol']
    processor.batch_process_contracts(contract_files=contract_files, output_file='function_level_dataset.json')
