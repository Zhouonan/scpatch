...
class SmartBugsProcessor:
    VULNERABILITY_TYPES = {'access_control': 'Access Control', 'arithmetic': 'Integer Overflow/Underflow', 'bad_randomness': 'Bad Randomness', 'denial_of_service': 'Denial of Service', 'front_running': 'Front Running', 'reentrancy': 'Reentrancy', 'time_manipulation': 'Time Manipulation', 'unchecked_low_level_calls': 'Unchecked Low Level Calls', 'short_addresses': 'Short Addresses', 'other': 'Other'}
    SOLIDIFI_MAPPING = {'Re-entrancy': 'reentrancy', 'Timestamp-Dependency': 'time_manipulation', 'Unhandled-Exceptions': 'unchecked_low_level_calls', 'Unchecked-Send': 'unchecked_low_level_calls', 'TOD': 'front_running', 'Overflow-Underflow': 'arithmetic', 'tx.origin': 'access_control'}

    def __init__(self, wild_dir: str=None, curated_dir: str=None, solidifi_dir: str=None, output_dir: str='./processed_data'):
        ...
        self.stats = {'wild': {'total': 0, 'valid': 0, 'errors': []}, 'curated': {'total': 0, 'valid': 0, 'errors': []}, 'solidifi': {'total': 0, 'valid': 0, 'errors': []}}
                ...
                for func in functions:
                    ...
                    if file_vulns:
                        for v in file_vulns:
                            ...
                            if matched_lines:
                                func['vulnerabilities'].append({'category': v['category'], 'lines': matched_lines})
                                func['is_vulnerable'] = True
                contract = ContractInfo(file_path=str(sol_file), contract_name=sol_file.stem, solidity_version=version, functions=functions, label={'has_vulnerability': True, 'vulnerability_type': vuln_type, 'vulnerability_name': self.VULNERABILITY_TYPES.get(vuln_type, vuln_type), 'source': 'curated', 'detailed_vulns': file_vulns}, metadata={'dataset': 'smartbugs_curated', 'ground_truth': True})
                contracts.append(contract)
                self.stats['curated']['valid'] += 1
            except Exception as e:
                self.stats['curated']['errors'].append({'file': str(sol_file), 'error': str(e)})
                    ...
                    for line in lines[start_idx:]:
                        ...
                        if len(parts) >= 3:
                            try:
                                ...
                                injections.append({'start_line': start_line, 'end_line': end_line, 'bug_type': b_type})
                            except ValueError:
                                continue
                line_offsets = [0]
                for (i, char) in enumerate(code):
                ...
                for func in functions:
                    ...
                    for inj in injections:
                        if not (inj['end_line'] < func_start_line or inj['start_line'] > func_end_line):
                            func['vulnerabilities'].append({'category': standard_bug_type, 'lines': [max(func_start_line, inj['start_line']), min(func_end_line, inj['end_line'])]})
                            func['is_vulnerable'] = True
                            contract_has_vuln = True
                contract = ContractInfo(file_path=str(sol_file), contract_name=sol_file.stem, solidity_version=version, functions=functions, label={'has_vulnerability': contract_has_vuln, 'vulnerability_type': raw_bug_type, 'vulnerability_name': standard_bug_type, 'source': 'solidifi', 'injections': injections}, metadata={'dataset': 'solidifi', 'ground_truth': True})
                contracts.append(contract)
                self.stats['solidifi']['valid'] += 1
            except Exception as e:
                self.stats['solidifi']['errors'].append({'file': str(sol_file), 'error': str(e)})
    ...
    def process_wild(self, sample_size: Optional[int]=None) -> List[ContractInfo]:
        ...
        for sol_file in tqdm(sol_files, desc='  处理中'):
            ...
            try:
                ...
                if not functions:
                    continue
                contract = ContractInfo(file_path=str(sol_file), contract_name=sol_file.stem, solidity_version=version, functions=functions, label=None, metadata={'dataset': 'smartbugs_wild', 'ground_truth': False})
                contracts.append(contract)
                self.stats['wild']['valid'] += 1
            except Exception as e:
                self.stats['wild']['errors'].append({'file': str(sol_file), 'error': str(e)})
    ...
    def extract_solidity_version(self, code: str) -> str:
        patterns = ['pragma\\s+solidity\\s+\\^?([0-9]+\\.[0-9]+\\.[0-9]+)', 'pragma\\s+solidity\\s+>=?([0-9]+\\.[0-9]+\\.[0-9]+)', 'pragma\\s+solidity\\s+([0-9]+\\.[0-9]+\\.[0-9]+)']
    ...
    def extract_functions(self, code: str) -> List[Dict]:
        ...
        for match in matches:
            ...
            visibility = 'public'
            if re.search('\\bprivate\\b', signature_str):
                visibility = 'private'
            elif re.search('\\binternal\\b', signature_str):
                visibility = 'internal'
            elif re.search('\\bexternal\\b', signature_str):
                visibility = 'external'
            state_mutability = 'nonpayable'
            if re.search('\\bpayable\\b', signature_str):
                state_mutability = 'payable'
            elif re.search('\\bpure\\b', signature_str):
                state_mutability = 'pure'
            elif re.search('\\bview\\b', signature_str):
                state_mutability = 'view'
            start = match.start()
            func_code = self.extract_function_body(code, start)
            functions.append({'name': func_name, 'visibility': visibility, 'state_mutability': state_mutability, 'code': func_code, 'start_pos': start})
        return functions
    ...
    def analyze_versions(self, wild_data: List[ContractInfo], curated_data: List[ContractInfo]):
        print('\n版本分布分析:')
        ...
        for (version, count) in curated_counter.most_common():
            ...
            print(f'  {version}: {count} ({percentage:.1f}%)')
        version_analysis = {'wild': dict(wild_counter), 'curated': dict(curated_counter)}
    ...
    def should_keep_contract(self, contract: ContractInfo) -> bool:
        ...
        if not contract.solidity_version:
            return False
        has_public_func = any((f['visibility'] in ['public', 'external'] for f in contract.functions))
    ...
    def create_dataset_splits(self, wild_data: List[ContractInfo], curated_data: List[ContractInfo]):
        print('\n创建数据集划分...')
        ...
        import random
        modern_samples = random.sample(wild_by_era['modern'], min(500, len(wild_by_era['modern'])))
        test_data.extend(modern_samples)
        splits = {'train': [c.file_path for c in train_data], 'val': [c.file_path for c in val_data], 'test': [c.file_path for c in test_data]}
    ...
    def save_contracts(self, contracts: List[ContractInfo], filename: str):
        ...
        for contract in contracts:
            data.append({'file_path': contract.file_path, 'contract_name': contract.contract_name, 'solidity_version': contract.solidity_version, 'num_functions': len(contract.functions), 'functions': contract.functions, 'label': contract.label, 'metadata': contract.metadata})
...
if __name__ == '__main__':
    processor = SmartBugsProcessor(wild_dir='/path/to/smartbugs-wild', curated_dir='/path/to/smartbugs-curated', output_dir='./processed_smartbugs')
