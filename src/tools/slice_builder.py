...
class CodeSliceBuilder:
    ...
    def build_slice(self, func_context: Dict) -> str:
        ...
        def _maybe_comment_modifier(code: str) -> str:
            ...
            if 'modifier ' in s or '{' in s or '}' in s or ('(' in s) or (')' in s) or ('\n' in s):
                return code
            if re.fullmatch('[A-Za-z_][A-Za-z0-9_]*', s):
                return f'// modifier: {s}'
            return code
    ...
    def build_simplified_contract(self, func_context: Dict, fixed_code: Optional[str]=None) -> str:
    ...
    def build_with_mode(self, func_context: Dict, mode: str='slice', fixed_code: Optional[str]=None) -> str:
        if mode == 'contract':
            return self.build_simplified_contract(func_context, fixed_code)
        else:
            return self.build_slice(func_context)

    def rebuild_full_contract(self, func_data: Dict, fixed_code: str) -> Optional[str]:
        import os
        ...
        if original_path and os.path.exists(original_path):
            try:
                ...
                if start_line is not None and end_line is not None and (start_line > 0) and (end_line <= len(lines)) and (start_line < end_line):
                    if self.include_comments:
...
if __name__ == '__main__':
    mock_context = {'function_name': 'withdraw', 'function_code': 'function withdraw(uint amount) public onlyOwner {\n    require(balances[msg.sender] >= amount);\n    _transfer(msg.sender, address(0), amount);\n    msg.sender.transfer(amount);\n}', 'solidity_version': '0.8.0', 'contract_context': {'contract_name': 'Vault', 'state_variables': [{'code': 'mapping(address => uint) public balances;', 'name': 'balances'}, {'code': 'address public owner;', 'name': 'owner'}], 'modifiers': [{'name': 'onlyOwner', 'code': 'modifier onlyOwner() {\n    require(msg.sender == owner);\n    _;\n}'}]}, 'called_functions': [{'name': '_transfer', 'code': 'function _transfer(address from, address to, uint amount) internal {\n    // Internal transfer logic\n    emit Transfer(from, to, amount);\n}'}]}
