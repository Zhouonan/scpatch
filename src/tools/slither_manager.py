...
TARGET_DETECTORS = {'UninitializedStateVarsDetection', 'UninitializedStorageVars', 'UninitializedLocalVars', 'UnusedStateVars', 'CouldBeConstant', 'CouldBeImmutable', 'VarReadUsingThis', 'PredeclarationUsageLocal', 'FunctionInitializedState', 'ConstantPragma', 'IncorrectSolc', 'LockedEther', 'UnprotectedUpgradeable', 'ConstantFunctionsAsm', 'ConstantFunctionsState', 'MissingInheritance', 'ArbitrarySendEth', 'Suicidal', 'ExternalFunction', 'UnimplementedFunctionDetection', 'DeadCode', 'ProtectedVariables', 'DomainSeparatorCollision', 'ChainlinkFeedRegistry', 'PythDeprecatedFunctions', 'OptimismDeprecation', 'ArbitrarySendErc20NoPermit', 'ArbitrarySendErc20Permit', 'IncorrectERC20InterfaceDetection', 'IncorrectERC721InterfaceDetection', 'UnindexedERC20EventParameters', 'ReentrancyBenign', 'ReentrancyReadBeforeWritten', 'ReentrancyEth', 'ReentrancyNoGas', 'ReentrancyEvent', 'TxOrigin', 'Assembly', 'LowLevelCalls', 'UnusedReturnValues', 'UncheckedTransfer', 'ControlledDelegateCall', 'Timestamp', 'MultipleCallsInLoop', 'IncorrectStrictEquality', 'DeprecatedStandards', 'RightToLeftOverride', 'TooManyDigits', 'UncheckedLowLevel', 'UncheckedSend', 'VoidConstructor', 'TypeBasedTautology', 'BooleanEquality', 'BooleanConstantMisuse', 'DivideBeforeMultiply', 'MappingDeletionDetection', 'ArrayLengthAssignment', 'RedundantStatements', 'BadPRNG', 'CostlyOperationsInLoop', 'AssertStateChange', 'WriteAfterWrite', 'MsgValueInLoop', 'DelegatecallInLoop', 'CacheArrayLength', 'IncorrectUsingFor', 'EncodePackedCollision', 'IncorrectOperatorExponentiation', 'TautologicalCompare', 'ReturnBomb', 'ChronicleUncheckedPrice', 'PythUncheckedConfidence', 'PythUncheckedPublishTime', 'ShadowingAbstractDetection', 'StateShadowing', 'LocalShadowing', 'BuiltinSymbolShadowing', 'ShiftParameterMixup', 'StorageSignedIntegerArray', 'UninitializedFunctionPtrsConstructor', 'ABIEncoderV2Array', 'ArrayByReference', 'EnumConversion', 'MultipleConstructorSchemes', 'PublicMappingNested', 'ReusedBaseConstructor', 'IncorrectReturn', 'ReturnInsteadOfLeave', 'MissingEventsAccessControl', 'MissingEventsArithmetic', 'MissingZeroAddressValidation', 'ModifierDefaultDetection', 'IncorrectUnaryExpressionDetection', 'OutOfOrderRetryable', 'GelatoUnprotectedRandomness'}
...
class SlitherManager:
    ...
    def extract_solidity_version(self, contract_file: str) -> Optional[str]:
        try:
            with open(contract_file, 'r', encoding='utf-8', errors='ignore') as f:
                content = f.read()
            pragma_matches = re.findall('pragma\\s+solidity\\s+([^;]+);', content, re.MULTILINE)
            if not pragma_matches:
                return None
            version_spec = pragma_matches[0].strip()
            version_matches = re.findall('(\\d+)\\.(\\d+)(?:\\.(\\d+))?', version_spec)
            ...
            if '~' in version_spec:
                ...
                return f'{major}.{minor}.{patch}'
            if '>=' in version_spec:
                ge_match = re.search('>=\\s*(\\d+)\\.(\\d+)(?:\\.(\\d+))?', version_spec)
                if ge_match:
                    (major, minor, patch) = ge_match.groups()
                    patch = patch if patch else '0'
                    return f'{major}.{minor}.{patch}'
            if '>' in version_spec and '>=' not in version_spec:
                gt_match = re.search('>\\s*(\\d+)\\.(\\d+)(?:\\.(\\d+))?', version_spec)
                if gt_match:
        ...
        try:
            if hasattr(solcx, 'get_executable'):
                try:
                    ...
                    pass
            home_dir = Path.home()
            possible_dirs = [home_dir / '.solcx' / f'solc-v{version}', home_dir / '.solcx' / 'bin' / f'solc-v{version}', home_dir / '.solcx' / f'solc-v{version}' / 'bin']
    ...
    def get_slither(self, contract_file: str, **kwargs) -> Optional[Slither]:
        ...
        try:
            ...
            slither_kwargs = {'solc_disable_warnings': True}
            if solc_path != 'SOLCX_ENV' and os.path.exists(solc_path):
    ...
    @contextmanager
    def analyze_contract(self, contract_file: str, **kwargs):
        slither = None
        try:
    ...
    def get_version_info(self, contract_file: str) -> Dict[str, str]:
        ...
        return {'pragma_version': pragma_version, 'feature_min_version': feature_version, 'recommended_version': max(pragma_version or '0.4.11', feature_version, key=lambda v: tuple((int(x) for x in v.split('.'))))}

def analyze_with_slither(contract_file: str, debug: bool=False, **kwargs):
