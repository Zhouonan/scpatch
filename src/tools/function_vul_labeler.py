...
class VulnerabilityCollector:
    DETECTOR_TO_VULN_TYPE = {'reentrancy-eth': 'reentrancy', 'reentrancy-no-eth': 'reentrancy', 'reentrancy-benign': 'reentrancy', 'reentrancy-events': 'reentrancy', 'reentrancy-unlimited-gas': 'reentrancy', 'suicidal': 'access_control', 'unprotected-upgrade': 'access_control', 'arbitrary-send-eth': 'access_control', 'arbitrary-send-erc20': 'access_control', 'arbitrary-send-erc20-permit': 'access_control', 'controlled-delegatecall': 'access_control', 'controlled-array-length': 'access_control', 'tx-origin': 'access_control', 'protected-vars': 'access_control', 'gelato-unprotected-randomness': 'access_control', 'incorrect-shift': 'arithmetic', 'incorrect-exp': 'arithmetic', 'divide-before-multiply': 'arithmetic', 'unchecked-lowlevel': 'unchecked_return', 'unchecked-send': 'unchecked_return', 'unchecked-transfer': 'unchecked_return', 'unused-return': 'unchecked_return', 'uninitialized-state': 'uninitialized_storage', 'uninitialized-storage': 'uninitialized_storage', 'uninitialized-local': 'uninitialized_storage', 'uninitialized-fptr-cst': 'uninitialized_storage', 'timestamp': 'timestamp_dependence', 'weak-prng': 'bad_randomness', 'delegatecall-loop': 'delegatecall', 'locked-ether': 'locked_ether', 'msg-value-loop': 'msg_value_loop', 'shadowing-state': 'shadowing', 'shadowing-abstract': 'shadowing', 'shadowing-builtin': 'shadowing', 'shadowing-local': 'shadowing', 'abiencoderv2-array': 'abi_encoding', 'encode-packed-collision': 'abi_encoding', 'array-by-reference': 'storage_issue', 'storage-array': 'storage_issue', 'mapping-deletion': 'storage_issue', 'incorrect-equality': 'logic_error', 'tautological-compare': 'logic_error', 'tautology': 'logic_error', 'boolean-cst': 'logic_error', 'incorrect-unary': 'logic_error', 'boolean-equal': 'logic_error', 'erc20-interface': 'interface_issue', 'erc721-interface': 'interface_issue', 'pyth-unchecked-confidence': 'oracle_issue', 'pyth-unchecked-publishtime': 'oracle_issue', 'chronicle-unchecked-price': 'oracle_issue', 'chainlink-feed-registry': 'oracle_issue', 'multiple-constructors': 'constructor_issue', 'reused-constructor': 'constructor_issue', 'void-cst': 'constructor_issue', 'incorrect-return': 'return_value_issue', 'return-leave': 'return_value_issue', 'pyth-deprecated-functions': 'deprecated_function', 'optimism-deprecation': 'deprecated_function', 'deprecated-standards': 'deprecated_standard', 'missing-zero-check': 'missing_validation', 'events-access': 'missing_event', 'events-maths': 'missing_event', 'erc20-indexed': 'event_issue', 'calls-loop': 'gas_optimization', 'costly-loop': 'gas_optimization', 'cache-array-length': 'gas_optimization', 'constable-states': 'gas_optimization', 'external-function': 'gas_optimization', 'immutable-states': 'gas_optimization', 'var-read-using-this': 'gas_optimization', 'assembly': 'assembly_usage', 'constant-function-asm': 'assembly_issue', 'low-level-calls': 'low_level_call', 'cyclomatic-complexity': 'code_quality', 'redundant-statements': 'code_quality', 'too-many-digits': 'code_quality', 'dead-code': 'dead_code', 'unused-state': 'unused_variable', 'enum-conversion': 'type_conversion', 'constant-function-state': 'state_mutability', 'name-reused': 'naming_issue', 'naming-convention': 'naming_convention', 'public-mappings-nested': 'visibility_issue', 'rtlo': 'malicious_code', 'codex': 'other', 'domain-separator-collision': 'signature_collision', 'unused-write': 'unused_write', 'out-of-order-retryable': 'transaction_order', 'incorrect-modifier': 'modifier_issue', 'variable-scope': 'scope_issue', 'return-bomb': 'gas_griefing', 'assert-state-change': 'assertion_issue', 'function-init-state': 'initialization_issue', 'incorrect-using-for': 'using_for_issue', 'missing-inheritance': 'inheritance_issue', 'pragma': 'pragma_issue', 'solc-version': 'compiler_version', 'unimplemented-functions': 'incomplete_implementation'}
    ...
    def collect_vulnerabilities(self) -> Dict[str, List[Dict]]:
        ...
        for result in self._flatten_results(results):
            ...
            for func_id in affected_functions:
                vulnerabilities_by_function[func_id].append({'type': vuln_type, 'detector': detector, 'severity': impact, 'description': description})
...
class FunctionVulnerabilityLabeler:
    ...
    def label_functions_in_contract(self, contract_file: str, include_safe: bool=True) -> List[Dict]:
        labels = []
        try:
            with self.manager.analyze_contract(contract_file) as slither:
                ...
                for contract in slither.contracts:
                    for function in contract.functions:
                        ...
                        label = {'contract_file': contract_file, 'contract_name': contract.name, 'function_name': function.name, 'function_signature': function.signature_str, 'function_code': self._get_function_code(function), 'is_vulnerable': len(func_vulns) > 0, 'vulnerability_types': vuln_types, 'vulnerability_count': len(vuln_types), 'vulnerability_details': func_vulns}
                        labels.append(label)
    ...
    def batch_label_contracts(self, contract_files: List[str], output_file: str, include_safe: bool=True, save_interval: int=100, debug: bool=False):
        all_labels = []
        stats = {'total_contracts': len(contract_files), 'processed_contracts': 0, 'failed_contracts': 0, 'total_functions': 0, 'vulnerable_functions': 0, 'safe_functions': 0, 'vulnerability_distribution': defaultdict(int), 'failed_files': []}
    ...
    def _print_stats(self, stats: Dict):
        ...
        if stats['vulnerability_distribution']:
            print(f'\n漏洞类型分布:')
            sorted_vulns = sorted(stats['vulnerability_distribution'].items(), key=lambda x: x[1], reverse=True)
...
def label_single_contract(contract_file: str, include_safe: bool=True, debug: bool=True) -> List[Dict]:
    labeler = FunctionVulnerabilityLabeler(debug=debug)
    return labeler.label_functions_in_contract(contract_file, include_safe)

def label_dataset(contract_dir: str, output_file: str, include_safe: bool=True, pattern: str='**/*.sol', debug: bool=False) -> tuple:
    contract_files = list(Path(contract_dir).glob(pattern))
    print(f'找到 {len(contract_files)} 个合约文件')
    labeler = FunctionVulnerabilityLabeler(debug=debug)
    return labeler.batch_label_contracts(contract_files=[str(f) for f in contract_files], output_file=output_file, include_safe=include_safe, debug=debug)
