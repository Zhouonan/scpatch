...
class LLMAnnotator:
    ...
    def __init__(self, config: AnnotationConfig):
        ...
        self.stats = {'total_requests': 0, 'successful_annotations': 0, 'failed_annotations': 0, 'total_tokens_used': 0, 'api_errors': 0}
    ...
    def _analyze_function(self, func_data: Dict, code_slice: str, slither_result: Dict) -> Optional[Dict]:
        ...
        prompt = self._build_prompt(code_slice=code_slice, slither_result=slither_result, label=func_data.get('label'), func_code=func_data.get('function_code', ''), func_start_line=func_start_line)
        if self.config.verbose:
            print('=' * 80)
            print('Prompt:')
            print('=' * 80)
    ...
    def _build_prompt(self, code_slice: str, slither_result: Dict, label: Dict, func_code: str='', func_start_line: int=1) -> str:
        prompt = f
                ...
                if vuln_details and len(vuln_details) > 0:
                    ...
                    for (i, vuln) in enumerate(vuln_details):
                        ...
                        if line_numbers and func_code:
                            vulnerable_code = self._extract_vulnerable_code(func_code, line_numbers, func_start_line)
                            prompt += f
                        elif line_numbers:
                            prompt += f
        if slither_result:
    ...
    def _select_target_vulnerability(self, vulnerability_details: List[Dict]) -> Dict:
        severity_order = {'High': 4, 'Medium': 3, 'Low': 2, 'Informational': 1, 'Optimization': 0}
        sorted_vulns = sorted(vulnerability_details, key=lambda v: severity_order.get(v.get('severity', 'Low'), 2), reverse=True)
    ...
    def _call_llm(self, prompt: str) -> Optional[str]:
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                api_params = {'model': self.config.model, 'messages': [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': prompt}], 'temperature': self.config.temperature}
    ...
    def _parse_response(self, response: str, slither_result: Dict) -> Optional[Dict]:
        try:
            ...
            annotation = {'label': label, 'analysis': analysis, 'reasoning': reasoning, 'vulnerability_types': data.get('vulnerability_types', []), 'severity': float(data.get('severity', 0.0)), 'confidence': float(data.get('confidence', 0.5)), 'suggested_fix': suggested_fix, 'slither_agreement': agrees_with_slither, 'slither_critique': slither_critique, 'metadata': {'model': self.config.model, 'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'), 'slither_result': 'vulnerable' if slither_says_vulnerable else 'safe', 'llm_result': label, 'agreement': agrees_with_slither}}
            if slither_result:
                if slither_says_vulnerable != (not is_safe):
                    annotation['metadata']['disagreement'] = {'slither': 'vulnerable' if slither_says_vulnerable else 'safe', 'llm': label, 'critique': slither_critique}
    ...
    def _parse_vulnerable_response(self, response: str, slither_result: Dict) -> Optional[Dict]:
        try:
            json_str = self._extract_json_from_response(response)
            if not json_str:
                print(f'无法从响应中提取JSON: {response[:200]}')
            ...
            if isinstance(suggested_fix, list):
                suggested_fix = '\n'.join((str(item) for item in suggested_fix))
            annotation = {'label': 'vulnerable', 'analysis': analysis, 'reasoning': reasoning, 'vulnerability_types': data.get('vulnerability_types', []), 'severity': float(data.get('severity', 5.0)), 'confidence': float(data.get('confidence', 0.5)), 'suggested_fix': suggested_fix, 'metadata': {'model': self.config.model, 'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'), 'slither_version': slither_result.get('version', 'unknown')}}
            self.stats['successful_annotations'] += 1
            return annotation
        except json.JSONDecodeError as e:
            print(f'JSON解析失败: {e}')
    ...
    def _parse_safe_response(self, response: str) -> Optional[Dict]:
        try:
            ...
            annotation = {'label': 'safe', 'analysis': analysis, 'reasoning': reasoning, 'vulnerability_types': [], 'severity': 0.0, 'confidence': float(data.get('confidence', 0.5)), 'best_practices': best_practices, 'metadata': {'model': self.config.model, 'timestamp': time.strftime('%Y-%m-%d %H:%M:%S')}}
            self.stats['successful_annotations'] += 1
            return annotation
        except json.JSONDecodeError as e:
            print(f'JSON解析失败: {e}')
    ...
    def batch_annotate(self, functions_data: List[Dict], progress_callback=None) -> List[Tuple[Dict, Optional[Dict]]]:
        results = []
...
if __name__ == '__main__':
    import os
    config = AnnotationConfig(api_key=os.getenv('OPENAI_API_KEY', 'your-api-key'), base_url=os.getenv('OPENAI_BASE_URL', 'https://api.openai.com/v1'), model='gpt-4o-mini', temperature=0.1)
    annotator = LLMAnnotator(config)
    mock_vulnerable_func = {'function_name': 'withdraw', 'function_code': 'function withdraw(uint amount) public {\n    msg.sender.call{value: amount}("");\n    balances[msg.sender] -= amount;\n}', 'contract_context': {'contract_name': 'Vault', 'state_variables': [{'code': 'mapping(address => uint) public balances;'}], 'modifiers': []}, 'called_functions': [], 'slither_result': {'is_vulnerable': True, 'vulnerability_details': [{'type': 'reentrancy-eth', 'severity': 'High', 'description': 'Reentrancy in withdraw function. External call before state update.', 'start_line': 10, 'end_line': 15, 'lines': [10, 11, 12, 13, 14, 15]}]}}
    print('测试标注器...')
