...
class PromptFormatter:
    ...
    def format_instruction(self) -> str:
        instruction = "Analyze the provided Solidity function code for security vulnerabilities. If the code is safe, output 'Safe' with a brief analysis. If vulnerabilities are detected, provide detailed information including: vulnerability type, severity score (0-10), reasoning, and location."
        return instruction
    ...
    def format_vulnerable_output(self, func_data: Dict, labels: Dict) -> str:
        ...
        output_data = {'reasoning': reasoning, 'type': vuln_type, 'severity': severity, 'location': location}
    ...
    def select_most_severe_vulnerability(self, vuln_details: List[Dict]) -> Dict:
        ...
        sorted_vulns = sorted(vuln_details, key=lambda v: severity_order.get(v.get('severity', 'Low'), 2), reverse=True)
        return sorted_vulns[0] if sorted_vulns else {}

    def convert_severity_to_score(self, severity_str: str, vuln_type: str) -> int:
        severity_mapping = {'High': 9, 'Medium': 6, 'Low': 3, 'Informational': 1, 'Optimization': 1}
        base_score = severity_mapping.get(severity_str, 5)
        type_adjustments = {'reentrancy': +1, 'delegatecall': +1, 'access_control': +1, 'arithmetic': 0, 'unchecked_call': 0, 'gas_optimization': -2, 'code_quality': -2}
    ...
    def extract_vulnerability_types(self, labels: Dict) -> List[str]:
        ...
        if labels.get('vulnerability_types'):
            ...
            vuln_types = [detail.get('type') for detail in labels['vulnerability_details'] if detail.get('type')]
    ...
    def generate_default_reasoning(self, vuln_type: str, func_data: Dict) -> str:
        reasoning_templates = {'reentrancy': 'The function performs external calls before updating state variables, which may allow reentrancy attacks.', 'access_control': 'The function lacks proper access control checks, allowing unauthorized users to execute sensitive operations.', 'arithmetic': 'The function contains arithmetic operations that may overflow or underflow.', 'unchecked_call': 'The function makes external calls without checking the return value.', 'delegatecall': 'The function uses delegatecall which may lead to unintended code execution.', 'tx_origin': 'The function uses tx.origin for authorization, which is vulnerable to phishing attacks.', 'timestamp_dependence': 'The function relies on block.timestamp which can be manipulated by miners.', 'uninitialized_storage': 'The function uses uninitialized storage variables.'}
        vuln_type_lower = vuln_type.lower().replace(' ', '_').replace('-', '_')
    ...
    def estimate_severity(self, vuln_type: str) -> int:
        severity_map = {'reentrancy': 9, 'access_control': 8, 'delegatecall': 9, 'arithmetic': 7, 'unchecked_call': 6, 'tx_origin': 7, 'timestamp_dependence': 5, 'uninitialized_storage': 8, 'dos': 6, 'front_running': 6}
        vuln_type_lower = vuln_type.lower().replace(' ', '_').replace('-', '_')
