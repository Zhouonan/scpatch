...
\
        ...
        self.stats = {
            ...
            'retry_successes': 0,                     
            'error_feedback_used': 0,                    
            'compilation_retry_successes': 0              
        }
...
\
        ...
        from src.tools.rag_retriever import (                
            HybridRetriever,
            RAGPromptBuilder,
            build_documents_from_fix_sft_jsonl,
            parse_vuln_info_from_text,
        ...
        builder = RAGPromptBuilder(
            max_demos=int(getattr(self.config, "rag_max_demos", 3)),
            max_chars_each=int(getattr(self.config, "rag_max_chars_each", 800)),
        )
...
\
    ...
    def _select_demos_with_token_budget(
        self,
        demos: List[Any],
        *,
        max_added_tokens: Optional[int],
    ) -> Tuple[List[Any], int]:
\
\
\
    ...
    def _maybe_apply_rag_to_prompt(
        self,
        *,
...
\
        ...
        for ex in selected:
            try:
                demo_summaries.append({
                    "vulnerability_type": str(getattr(ex, "vulnerability_type", "Unknown")),
                    "score": float(getattr(ex, "score", 0.0)),
                    "code_preview": (getattr(ex, "code", "") or "")[:200],
                    "fixed_code_preview": (getattr(ex, "fixed_code", "") or "")[:200],
                })
            except Exception:
                pass
        ...
        rag_meta = {
            "rag_used": True,
            "rag_top_k": int(getattr(self.config, "rag_top_k", 3)),
            "rag_num_demos": len(selected),
            "rag_vuln_type_filter": vt,
            "rag_index_path": getattr(self.config, "rag_index_path", None),
            "rag_max_added_tokens": max_added,
            "rag_added_tokens_est": int(added_tokens),
            "rag_demo_summaries": demo_summaries,
        }
...
\
        ...
        for attempt in range(1, self.config.max_fix_attempts + 1):
            ...
            fixed_code, fix_analysis = self._generate_fixed_code(
                func_data=func_data,
            ...
            if attempt == 1 and fix_analysis:
                first_fix_metadata = {
                    ...
                    'key_changes': annotation.get('key_changes', []),
                    'solidity_version_notes': f"Compatible with Solidity {func_data.get('solidity_version', '0.8.0')}"
                }
            
                         
            result_metadata = first_fix_metadata if first_fix_metadata else {
                'fix_analysis': fix_analysis,
                'key_changes': annotation.get('key_changes', []),
                'solidity_version_notes': f"Compatible with Solidity {func_data.get('solidity_version', '0.8.0')}"
            }
            ...
            result = {
                'original_code': code_slice,
                'fixed_code': fixed_code,
                                                            
                'fix_analysis': result_metadata['fix_analysis'],
                'fix_explanation': result_metadata['fix_analysis'],
                'vulnerabilities_fixed': annotation.get('vulnerability_types', []),
                'verification': verification_result,
                'metadata': {
                    'model': self.config.model,
                    'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'),
                    'attempts': attempt,
                    'original_severity': annotation.get('severity', 0),
                    'used_error_feedback': previous_fixed_code is not None,
                    'key_changes': result_metadata['key_changes'],
                    'solidity_version_notes': result_metadata['solidity_version_notes']
                }
            }
            ...
            if self._is_fix_acceptable(verification_result):
                ...
                self.stats['fix_attempts_distribution'][attempt] =\
                    self.stats['fix_attempts_distribution'].get(attempt, 0) + 1
...
\
    ...
    def _generate_fixed_code(
        ...
        attempt: int,
        solc_version: str,
        previous_fixed_code: Optional[str] = None,
...
\
        ...
        is_retry = bool(
            attempt > 1
            and self.config.enable_error_feedback
            and previous_fixed_code
            and previous_error
        )
    ...
    def _build_fix_prompt(
        ...
        if self.config.evaluation_mode and not (attempt > 1 and previous_error):
            ...
            return self.prompt_formatter.format_fix_prompt_for_our_models(
                removed_comments_code_slice, 
                annotation, 
                solc_version,
        ...
        if (attempt > 1 and self.config.enable_error_feedback and previous_fixed_code and previous_error):
            ...
            prompt = f"""You are an expert Solidity security engineer. Your previous fix attempt failed verification.
...
**Your Task:**
Update the function to address all vulnerabilities AND resolve the verification findings above.
...
"""
        ...
        if self.config.use_rich_fix_prompt:
            return self.prompt_formatter.format_general_fix_prompt_rich(
                code_slice=code_slice,
                annotation=annotation,
                solc_version=solc_version,
            )
        return self.prompt_formatter.format_general_fix_prompt(
            code_slice=code_slice,
            annotation=annotation,
            solc_version=solc_version,
        )
    ...
    def _remove_annotation_comments(self, code: str) -> str:
        ...
        code = re.sub(r'//\s*<yes>\s*<report>\s*[A-Z_]+', '', code)
        
                                    
        code = re.sub(r'//\s*<yes>\s*<report>', '', code)
        
                                
        code = re.sub(r'//\s*VULNERABLE', '', code)
    ...
    def _build_retry_prompt(
        ...
        return self.prompt_formatter.format_retry_prompt(
            previous_fixed_code=previous_fixed_code,
    ...
    def _extract_error_message(self, error_text: str) -> str:
        ...
        match = re.search(r'Error:\s*(.+)$', error_text, re.MULTILINE)
        if match:
...
\
        ...
        if self.config.verbose:
            print(
                f"⚠️  stop 序列过长({len(deduped)}), 已自动裁剪为 {len(trimmed)}: {trimmed}"
            )
        return trimmed
    ...
    def _is_n_only_one_error(self, e: Exception) -> bool:
        ...
        return (
            ("only n = 1" in msg)
            or ("only n=1" in msg)
            or ("invalid n value" in msg)
            or ("range of n should be" in msg)
            or ("invalid 'n'" in msg)
            or ("invalid n" in msg and "n" in msg and "supported" in msg)
        )

    def _call_llm_n_fallback(
        self,
        request_kwargs: Dict,
        n: int,
    ) -> List[str]:
...
\
        ...
        for retry in range(self.config.max_retries):
            try:
                ...
                if desired_n > 1 and self._should_force_single_n():
                    if self.config.verbose:
                        print(
                            f"ℹ️  Model '{self.config.model}' 命中 force_single_n_model_keywords，"
                            f"将 n={desired_n} 改为循环调用 {desired_n} 次（每次 n=1）"
                        )
                    request_kwargs["n"] = 1
                    ...
                    try:
                        ...
                        if (
                            self.config.allow_n_fallback
                            and isinstance(n, int)
                            and n > 1
                            and self._is_n_only_one_error(e)
                        ):
                            if self.config.verbose:
    ...
    def _call_llm(
        ...
        if self.config.evaluation_mode:
            return self._call_llm_for_evaluation(
                prompt,
                temperature,
                is_retry,
                n,
        ...
        for retry in range(self.config.max_retries):
            try:
                ...
                request_kwargs = {
                    ...
                    "response_format": {'type': 'json_object'} if (not is_retry and not self.config.evaluation_mode) else None,
                }
                if self.config.top_p is not None:
                    request_kwargs["top_p"] = self.config.top_p
                if self.config.max_tokens is not None:
    ...
    def _extract_code_from_response(self, response: str) -> Optional[str]:
        ...
        code_block = re.search(r'```(?:solidity)?\s*(.*?)\s*```', response, re.DOTALL)
        if code_block:
        ...
        if self.config.enable_mythril_check:
            ...
            if isinstance(full_contract, str) and full_contract.strip():
                try:
                    ...
                    mr = self.mythril_manager.analyze_source(
                        contract_src=full_contract,
                        timeout=int(self.config.mythril_timeout),
                        severities=self.config.mythril_severities if self.config.mythril_severities is not None else ("high", "medium"),
                        max_issues=50,
                    )
...
\
    ...
    def _check_with_slither(
        self,
        func_data: Dict,
        fixed_code: str
    ) -> Tuple[bool, List[str], float, float, bool]:
\
\
\
\
...
\
        ...
        return "|".join(
            [
                str(func_data.get("contract_path") or ""),
                str(func_data.get("function_name") or ""),
                str(func_data.get("start_line") or ""),
                str(func_data.get("end_line") or ""),
                str(func_data.get("solidity_version") or ""),
            ]
        )
...
\
    ...
    def batch_generate_fixes(
        self,
        annotated_data: List[Tuple[Dict, Dict]],
        progress_callback=None
    ) -> List[Tuple[Dict, Dict, Optional[Dict]]]:
\
\
\
\
...
\
        ...
        vulnerable_data = [
            (func_data, annotation)
            for func_data, annotation in annotated_data
            if annotation and annotation.get('label') == 'vulnerable'
        ]
    ...
    def print_stats(self):
        ...
        if self.stats['successful_fixes'] > 0:
            ...
            success_rate = (self.stats['successful_fixes'] / 
                          (self.stats['successful_fixes'] + self.stats['failed_fixes'])) * 100
...
if __name__ == "__main__":
    ...
    mock_annotation = {
        "label": "vulnerable",
        "analysis": "This function is vulnerable to reentrancy attacks.",
        "reasoning": "The function makes an external call before updating the state.",
        "vulnerability_types": ["reentrancy-eth"],
        "severity": 9.0,
        "confidence": 0.95
    }
