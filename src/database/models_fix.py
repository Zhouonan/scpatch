...
class VulnerabilityFix(Base):
    ...
    function_id = Column(Integer, ForeignKey('smart_contract_functions.id'), 
                        nullable=False, index=True, comment="关联的函数ID")
    ...
    __table_args__ = (
        Index('idx_function_version', 'function_id', 'fix_version'),
        Index('idx_fix_quality', 'compiles', 'slither_passed'),
...
\
    ...
    fix_id = Column(Integer, ForeignKey('vulnerability_fixes.id'), 
                   index=True, comment="关联的修复记录ID")
    function_id = Column(Integer, ForeignKey('smart_contract_functions.id'),
                        index=True, comment="关联的函数ID")
    ...
    __table_args__ = (
        Index('idx_dataset_split', 'dataset_name', 'is_training', 'is_validation', 'is_test'),
        Index('idx_pair_quality', 'quality_checked', 'quality_score'),
    )
