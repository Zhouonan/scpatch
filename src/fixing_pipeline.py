...
class FixingPipeline:
    ...
    def __init__(
        ...
        failure_log_dir: str = "failure_logs",
        output_json: bool = False,
        output_json_dir: str = "fix_json_outputs",
        skip_existing_json: bool = True,
        output_json_mode: str = "failed",
        output_json_full: bool = False,
                                                                                                         
        use_label_annotation: bool = False,
            ...
            for coro in asyncio.as_completed(tasks):
                ...
                if status == 'no_annotation':
                    ...
                    pbar.set_postfix({
                        ...
                        'current': f"SKIP: {contract_name}.{func_name}"
                    })
                elif status == 'safe':
                    skipped_count += 1
                    pbar.set_postfix({
                        ...
                        'failed': failed_count,
                        'skipped': skipped_count,
                        'current': f"SAFE: {contract_name}.{func_name}"
                    })
                    ...
                    if fix_result:
                        ...
                        pbar.set_postfix({
                            ...
                            'failed': failed_count,
                            'skipped': skipped_count,
                            'current': f"ERROR: {contract_name}.{func_name}"
                        })
...
\
        ...
        if self.use_label_annotation:
            functions = self._query_functions_label_based(
                ...
                skip_fixed_scope=skip_fixed_scope,
                fixed_model=(self.fixer.config.model if skip_fixed_scope == "model" else None),
    ...
    def _query_functions_label_based(
        self,
        dataset_types: Optional[List[str]],
        dataset_names: Optional[List[str]],
        only_vulnerable: bool,
    ...
    def _prepare_function_data(self, functions: List) -> List[Dict]:
        ...
        for func in functions:
            func_data = {
                ...
                'dataset_name': getattr(func, 'dataset_name', None),
                'dataset_type': getattr(func, 'dataset_type', None),
                'contract_name': getattr(func, 'contract_name', None),
                ...
                'label': func.label,
                'llm_audit': func.llm_audit or {},
                'solidity_version': func.solidity_version or '0.8.0'
            }
    ...
    def _save_fixes(self, functions_data: List[Dict]):
        ...
        for func_data in functions_data:
            ...
            fix_data = {
                ...
                'sample_id': sample_id,
                'original_code': llm_fix.get('original_code'),
                'fixed_code': llm_fix.get('fixed_code'),
                                                             
                'fix_explanation': llm_fix.get('fix_explanation') or llm_fix.get('fix_analysis'),
                'vulnerabilities_fixed': llm_fix.get('vulnerabilities_fixed', []),
                'original_severity': metadata.get('original_severity'),
                'compiles': verification.get('compiles', False),
                'slither_passed': bool(tools_passed),
                'remaining_issues': verification.get('remaining_issues', []),
                'verification_details': verification,
                'model_name': metadata.get('model'),
                'fix_attempts': metadata.get('attempts', 1),
                'raw_fix_data': llm_fix
            }
...
\
        ...
        return {
            "ids": {
                "function_db_id": func_data.get("_db_id"),
                "sample_id": func_data.get("_sample_id"),
            },
            "result": {
                "status": func_data.get("_fix_status"),
                "failure_reason": func_data.get("_fix_failure_reason"),
            },
            "source": {
                "dataset_name": func_data.get("dataset_name"),
                "dataset_type": func_data.get("dataset_type"),
                "contract_path": func_data.get("contract_path"),
                "contract_name": func_data.get("contract_name") or contract_ctx.get("contract_name"),
                ...
                "solidity_version": func_data.get("solidity_version"),
            },
            "audit": {
                "label": llm_audit.get("label"),
                "severity": llm_audit.get("severity"),
                "vulnerability_types": llm_audit.get("vulnerability_types") or llm_audit.get("vulnerability_type"),
            ...
            "fix": {
                ...
                "vulnerabilities_fixed": llm_fix.get("vulnerabilities_fixed", []),
                "original_function_code": func_data.get("function_code"),
                "fixed_code": llm_fix.get("fixed_code"),
                "fix_explanation": llm_fix.get("fix_explanation") or llm_fix.get("fix_analysis"),
                "verification": llm_fix.get("verification", {}),
            },
...
\
        ...
        if self.enable_mythril_check:
            ...
            if isinstance(tool_results.get("mythril"), dict):
                ...
                mythril_tool = {
                    "passed": bool(verification.get("mythril_passed", True)),
                    "issues": verification.get("mythril_issues", []),
                    "error": verification.get("mythril_error"),
                }
                ...
                if isinstance(full_contract, str) and full_contract.strip():
                    res = self.mythril_manager.analyze_source(
                        contract_src=full_contract,
                        timeout=int(self.mythril_timeout),
                        severities=self.mythril_severities if self.mythril_severities is not None else ("high", "medium"),
                        max_issues=50,
                    )
                    mythril_tool = {
                        "passed": bool(res.passed),
                        "issue_count": int(res.issue_count),
                        "issues": res.issues,
                        "error": res.error,
...
def main():
    ...
    parser.add_argument(
        ...
        choices=['model', 'any'],
        default='model',
        help='跳过已修复数据的范围：model=仅当前--model修复过才跳过（推荐，避免同模型重复）；any=任意模型修复过就跳过（旧行为）'
    )
    parser.add_argument(
        '--min-severity',
        type=float,
    ...
    parser.add_argument(
        '--disable-compilation-check',
        action='store_true',
        help='禁用编译检查'
    )
    parser.add_argument(
        '--disable-slither-check',
        action='store_true',
        help='禁用Slither检查'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--output-json',
        action='store_true',
        help='将修复结果输出为单独的json文件（默认只输出失败，可用 --output-json-mode 调整）'
    )
    parser.add_argument(
        '--output-json-dir',
        type=str,
        default='fix_json_outputs',
        help='json文件输出目录（配合--output-json使用）'
    )
    parser.add_argument(
        '--output-json-mode',
        type=str,
        choices=['failed', 'all', 'success'],
        default='failed',
        help='json输出模式：failed=仅失败（默认）；all=全部；success=仅成功'
    )
    parser.add_argument(
        '--no-skip-existing-json',
        action='store_true',
        help='不跳过已存在的单条json输出；若文件已存在将自动追加后缀保存（默认会跳过以避免重复）'
    )
    parser.add_argument(
        '--output-json-full',
        action='store_true',
        help='--output-json 输出全量func_data（包含上下文/切片等大字段）；默认输出精简的“修复相关”字段'
    )
    parser.add_argument(
        '--enable-mythril-check',
        action='store_true',
        help='启用Mythril检查（基于verification.full_contract）。默认关闭以避免环境缺少mythril导致额外开销'
    )
    parser.add_argument(
        '--mythril-timeout',
        type=int,
    ...
    parser.add_argument(
        ...
        nargs='*',
        default=None,
        help="Mythril严重性过滤（默认: high medium），例如: --mythril-severities high medium"
    )
    parser.add_argument(
        '--mythril-bin',
        type=str,
        default='myth',
        help='Mythril可执行文件名/路径（默认: myth）'
    )
    parser.add_argument(
        '--verbose',
        action='store_true',
    ...
    parser.add_argument(
        '--use-label-annotation',
        action='store_true',
        help='当 llm_audit 缺失时，使用数据库中的 ground-truth label + slither_result 构造伪 annotation 进行修复（默认关闭，不影响原逻辑）'
    )
    parser.add_argument(
        '--use-rich-fix-prompt',
        action='store_true',
        help='启用 richer 修复 prompt（包含漏洞类型、漏洞行片段、Slither 报告等）。默认关闭，不影响原 prompt。'
    )
    parser.add_argument(
        '--vuln-types',
        type=str,
        ...
        default=None,
        help=(
            "可选：仅选择指定漏洞类型进行修复/数据生成。支持 ScrawlD 映射中的 "
            "category（如 reentrancy）、key（如 REENTRANCY）、token（如 RENT）。"
            "不传则不筛选（默认行为不变）。"
        ),
    )
    ...
    config = FixerConfig(
        ...
        enable_compilation_check=not args.disable_compilation_check,
        enable_slither_check=not args.disable_slither_check,
        solc_version=args.solc_version,
        max_fix_attempts=args.max_fix_attempts,
        enable_mythril_check=args.enable_mythril_check,
        mythril_timeout=args.mythril_timeout,
        mythril_severities=args.mythril_severities,
        mythril_bin=args.mythril_bin,
        use_rich_fix_prompt=getattr(args, "use_rich_fix_prompt", False),
    )
    ...
    pipeline = FixingPipeline(
        ...
        skip_existing_json=not args.no_skip_existing_json,
        output_json_mode=args.output_json_mode,
        output_json_full=args.output_json_full,
        use_label_annotation=getattr(args, "use_label_annotation", False),
        enable_mythril_check=args.enable_mythril_check,
        mythril_timeout=args.mythril_timeout,
        mythril_severities=args.mythril_severities,
        mythril_bin=args.mythril_bin
    ...
    try:
        pipeline.run(
            ...
            limit=args.limit,
            only_vulnerable=not args.include_safe,
            skip_already_fixed=not args.refix,
            skip_fixed_scope=args.skip_fixed_scope,
            min_severity=args.min_severity,
            vuln_types=getattr(args, "vuln_types", None),
        )
